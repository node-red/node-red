 ...
 ******************************************************************************/
    ...
    flowFile: '/dev/null', // Flows are not saved to disk in sandbox mode
     ...
     */
    userDir: '/tmp/node-red-sandbox/', // Use temp directory for sandbox
 ...
 ******************************************************************************/
     ...
     */
    // Authentication disabled for public sandbox mode
    //adminAuth: {
    //    type: "credentials",
    //    users: [{
    //        username: "admin",
 ...
 ******************************************************************************/
    ...
    httpAdminMiddleware: function (req, res, next) {
        // Sandbox mode middleware - allow UI operations but prevent harmful actions
        
        // Allow flows endpoint for client-side deploy (returns success but doesn't save)
        if (req.path === '/flows' && req.method === 'POST') {
            // Return success to keep UI happy, but don't actually save
            return res.json({ rev: "sandbox-" + Date.now() });
        }
        
        // Block dangerous operations
        const dangerousOperations = [
            { path: '/nodes/', methods: ['POST', 'PUT', 'DELETE'] }, // Node installation
            { path: '/library/', methods: ['POST', 'PUT', 'DELETE'] }, // Library modifications
            { path: '/projects/', methods: ['POST', 'PUT', 'DELETE'] }, // Project operations
            { path: '/credentials/', methods: ['GET', 'POST', 'PUT', 'DELETE'] }, // Credentials
            { path: '/flows/state', methods: ['POST', 'PUT'] }, // Runtime state changes
        ];
        
        for (const rule of dangerousOperations) {
            if (req.path.includes(rule.path) && rule.methods.includes(req.method)) {
                return res.status(403).json({
                    error: "Operation not available in sandbox",
                    message: "This operation is disabled in sandbox mode"
                });
            }
        }
        
        // Add security headers
        res.setHeader('X-Frame-Options', 'SAMEORIGIN'); // Allow embedding if needed
     ...
     */
    httpNodeCors: {
        origin: false, // Disable CORS for HTTP nodes in demo mode
        methods: "GET" // Only allow GET if somehow enabled
    },
    ...
    httpNodeMiddleware: function(req,res,next) {
        // Block all HTTP in node endpoints in demo mode
        return res.status(403).json({
 ...
 ******************************************************************************/
    ...
    */
    ...
    /** Disable the actual execution of flows */
    nodesDir: '/tmp/empty-nodes-dir', // Point to empty dir to prevent loading custom nodes
    autoInstallModules: false, // Never auto-install anything
    telemetry: {
        /**
    ...
    contextStorage: {
        default: {
            module: "memory" // Use memory-only storage for demo mode (no persistence)
        },
        // Disable file system storage completely
        store: {
            module: "memory"
        }
    },
    ...
    externalModules: {
        ...
        palette: {              /** Configuration for the Palette Manager */
            allowInstall: false, /** Disable the Palette Manager in demo mode */
            allowUpdate: false,  /** Disallow module updates in demo mode */
            allowUpload: false,  /** Disallow module uploads in demo mode */
            allowList: [],
            denyList: ['*'], // Deny all module operations
            allowUpdateList: [],
            denyUpdateList: ['*']
        },
        modules: {              /** Configuration for node-specified modules */
            allowInstall: false,
            allowList: [],
            denyList: ['*'] // Deny all external module loads
        }
    },
 ...
 ******************************************************************************/
     ...
     */
    fileWorkingDirectory: "/tmp/node-red-demo", // Restrict file operations to a safe directory

    /** Allow the Function node to load additional npm modules directly */
    functionExternalModules: false, // Disabled in demo mode for security
     ...
     */
    globalFunctionTimeout: 5, // 5 second timeout for all functions in demo mode
     ...
     */
    functionTimeout: 5, // 5 second timeout for functions in demo mode
    ...
    execMaxBufferSize: 0, // Disable exec node completely by setting buffer to 0
     ...
     */
    tlsConfigDisableLocalFiles: true, // Disable local file access for TLS configs
    ...
    webSocketNodeVerifyClient: function(info) {
        /** In demo mode, we reject all WebSocket connections to prevent
         * any real-time communication or potential security issues
         */
        console.log('WebSocket connection attempt blocked in demo mode');
        return false; // Reject all WebSocket connections in demo mode
    },
}
