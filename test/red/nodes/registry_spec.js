...
describe('NodeRegistry', function() {
    it('does not accept incorrect nodesDir',function(done) {
        ...
        typeRegistry.load().then(function(){
            try {
                assert.fail(null, null, "Loading of non-existing nodesDir should never succeed");
            } catch (err) {
                done(err);
            }
...
describe("getNodeFiles", function() {
    ...
    it('loads additional node files from specified external nodesDir',function(done) {
        ...
        var settings = {
                nodesDir : tempDir,
                defaultNodesDir : tempNoNodesContainedDir
        }
        ...
        typeRegistry.load().then(function(){
            fs.exists(nodeInjectedFilePath, function(exists) {
               if(exists) {
                   ...
                   try {
                       assert.fail(null, null, nodeInjectedFilePath + " should be created by registered test node.");   
                   } catch(err) {
                       done(err)
                   }
               }
            ...
            try {
                assert.fail(null, null, "Loading of nodesDir should succeed");
            } catch (err) {
                done(err);
            }
        });
