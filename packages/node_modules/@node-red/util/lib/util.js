...
/**
 * Gives every Buffer in an already-cloned value its own backing store.
 *
 * lodash.clonedeep clones a Buffer with `buffer.slice()`, which returns a view
 * over the same memory rather than a copy. The clone therefore shares bytes with
 * the original, so two flow branches that mutate a Buffer payload in place
 * corrupt each other. Every other binary type (TypedArray, ArrayBuffer,
 * DataView) is already copied correctly by lodash and is left alone here.
 *
 * The value passed in is the fresh clone, so it is safe to repair in place.
 *
 * @param  {any}     value - a value from the cloned message
 * @param  {Set}     seen  - objects already visited, to survive cycles
 * @return {any}     the value, with any Buffers replaced by owned copies
 */
function isolateBuffers(value, seen) {
    ...
    // already copied by lodash, and not containers we need to walk into
    ...
    if (Array.isArray(value)) {
        for (let i = 0; i < value.length; i++) {
            value[i] = isolateBuffers(value[i], seen);
        }
        return value;
    }
    if (value instanceof Map) {
        for (const [key, entry] of value) {
            value.set(key, isolateBuffers(entry, seen));
        }
        return value;
    }
    if (value instanceof Set) {
        ...
        for (const entry of entries) {
            value.add(isolateBuffers(entry, seen));
        }
        return value;
    }
    for (const key of Object.keys(value)) {
        try {
            value[key] = isolateBuffers(value[key], seen);
        } catch (err) {
            // a read-only or throwing accessor: leave the property as it is
        }
    }
    return value;
}
...
function cloneMessage(msg) {
    if (typeof msg !== "undefined" && msg !== null) {
        ...
        delete msg.req;
        delete msg.res;
        var m = isolateBuffers(clonedeep(msg), new Set());
        if (req) {
            m.req = req;
            msg.req = req;
        }
...
function prepareJSONataExpression(value,node) {
    ...
    expr.assign('moment', function(arg1, arg2, arg3, arg4) {
        const result = moment(arg1, arg2, arg3, arg4);
        return result;
