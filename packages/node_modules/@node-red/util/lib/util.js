...
function prepareJSONataExpression(value,node) {
    ...
    expr.assign('has', function(collection, key) {
        // NOTE: instanceof is not returning Set or Map, so fallback to internal [[Class]] prop
        const typeString = Object.prototype.toString.call(collection);
        if (typeString === '[object Set]' || typeString === '[object Map]') {
            return collection.has(key);
        } else {
            throw new Error("$has expects a Set or Map as the first argument");
        }
    });
