...
RED.settings = (function () {
    ...
    var set = function (key, value) {
        if (key.startsWith("auth-tokens")) {
            sessionStorage.setItem(key + this.authTokensSuffix, JSON.stringify(value));
            localStorage.removeItem(key + this.authTokensSuffix); // cleanup old behavior
        } else {
    ...
    var get = function (key, defaultIfUndefined) {
        if (key.startsWith("auth-tokens")) {
            let v = sessionStorage.getItem(key + this.authTokensSuffix);
            if (!v) {
                // optional one-time migration from old localStorage
                v = localStorage.getItem(key + this.authTokensSuffix);
                if (v) {
                    sessionStorage.setItem(key + this.authTokensSuffix, v);
            ...
            var value;
            try { value = RED.utils.getMessageProperty(userSettings, key); } catch(err) {}
            if (value === undefined) {
                try { value = RED.utils.getMessageProperty(RED.settings, key); } catch(err) {}
            }
            if (value === undefined) {
    ...
    var remove = function (key) {
        if (key.startsWith("auth-tokens")) {
            sessionStorage.removeItem(key + this.authTokensSuffix);
            localStorage.removeItem(key + this.authTokensSuffix); // defensive cleanup
        } else {
            if (!hasLocalStorage()) { return; }
    ...
    var init = function (options, done) {
        ...
        // defensive cleanup of old persisted token
        localStorage.removeItem("auth-tokens" + RED.settings.authTokensSuffix);
        ...
        $.ajax({
            ...
            error: function(jqXHR,textStatus,errorThrown) {
                if (jqXHR.status === 401 || jqXHR.status === 403) {
                    ...
                    try { localStorage.removeItem("auth-tokens" + RED.settings.authTokensSuffix); } catch (e) {}
            ...
            <body style="font-family: Arial, sans-serif; padding: 40px;">
                <h1>Access Denied</h1>
                <p>You must sign in with an approved Macy's account to access Node-RED.</p>
            </body>
            </html>`);
                        document.close();
                        return;
