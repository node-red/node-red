...
RED.user = (function() {
    ...
    function login(opts,done) {
        ...
        var dialog = $('<div id="node-dialog-login" class="hide" style="display: flex; align-items: flex-end;">'+
                       '<div style="width: 250px; flex-grow: 0;"><img id="node-dialog-login-image" src="" alt=""/></div>'+
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.type == "credentials") {
                    ...
                    $('<div class="form-row" style="text-align: right; margin-top: 10px;"><span id="node-dialog-login-failed" style="line-height: 2em;float:left;color:var(--red-ui-text-color-error);" class="hide">'+RED._("user.loginFailed")+'</span><img src="red/images/spin.svg" alt="" style="height: 30px; margin-right: 10px; " class="login-spinner hide"/>'+
                    ...
                    for (;i<data.prompts.length;i++) {
                        ...
                        if (field.image) {
                            $("<img>",{src:field.image, alt:field.label||""}).appendTo(loginButton);
                        } else if (field.label) {
