RED.multiplayer = (function () {
    ...
    // This is generated by the client and not shared to other multiplayer clients
    let activeSessionId

    // publicSessionId - used to publicly identify this session.
    // This is generated by the server provided to the client on connect/reconnect.
    // Any other multiplayer clients will use this to identify this session.
    let publicSessionId
    ...
    function addUserSession (session) {
        ...
        if (session.session === publicSessionId) {
            ...
            workspaceTrays[workspaceId] = {
                ...
                addUser: function (sessionId) {
                    if (users.indexOf(sessionId) === -1) {
                        ...
                        const userLocationIcon = $(`<div class="red-ui-multiplayer-user-location"></div>`).attr("id", userLocationId)
    ...
    function updateUserLocation (sessionId, location) {
        ...
        if (location.workspace) {
            ...
            if (location.cursor && location.workspace === RED.workspaces.active() && !isNaN(location.cursor.x) && !isNaN(location.cursor.y)) {
                if (!sessions[sessionId].cursor) {
        ...
        init: function () {
            ...
            RED.comms.subscribe('multiplayer/#', (topic, msg) => {
                ...
                if (topic === 'multiplayer/init') {
                    ...
                    publicSessionId = msg.session
                    log('publicSessionId', publicSessionId)
