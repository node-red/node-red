        ...
        $.getJSON(options.apiRootUrl+"theme", function(theme) {
            if (theme.header) {
                ...
                if (theme.header.title) {
                    // Enhanced header with dashboard link and issue linking
                    var headerContainer = $('<span>').appendTo(logo);
                    
                    // Extract branch name and check for issue pattern
                    var branchMatch = theme.header.title.match(/Node-RED.*?-\s*(.+)$/);
                    var branchName = branchMatch ? branchMatch[1] : '';
                    var issueMatch = branchName.match(/^issue-(\d+)$/);
                    var issueNumber = issueMatch ? issueMatch[1] : null;
                    
                    if (issueNumber) {
                        // If branch follows issue-XXXX pattern, link to GitHub issue
                        headerContainer.html('Node-RED Experiment - ')
                            .append($('<a>', {
                                href: 'https://github.com/node-red/node-red/issues/' + issueNumber,
                                target: '_blank',
                                text: 'Issue #' + issueNumber,
                                style: 'color: inherit; text-decoration: underline;'
                            }));
                    } else {
                        // Otherwise, just show the title with branch name
                        headerContainer.html(theme.header.title);
                    }
                    
                    // Add dashboard link
                    headerContainer.append(' | ')
                        .append($('<a>', {
                            href: 'https://dashboard.tailbfedba.ts.net',
                            target: '_blank',
                            text: 'Dashboard',
                            style: 'color: inherit; text-decoration: underline;'
                        }));
