...
RED.nodes = (function() {
     ...
     **/
    function convertNode(n, exportCreds, toClipboard) {
        if (n.type === 'tab') {
        ...
        if (n.type.substring(0,8) == "subflow:") {
            ...
            var sf = getSubflow(subflowId);
            if (toClipboard && sf.sealed && !sf.imported) {
                node.type = "subflow:" +sf.exportName;
            }
        }
        if (n._def.category != "config") {
            node.x = n.x;
    ...
    function collectNodesInSubflow(n, isSealed, toClipboard) {
        var id = n.id;
        var nodes = RED.nodes.filterNodes({z:id});
        var set = createExportableNodeSet(nodes, undefined, undefined, undefined, toClipboard);
        var newSet = [];
    ...
    function convertSubflow(n, exportCreds, isSealed, toClipboard) {
        var node = {};
        node.id = n.id;
        node.type = n.type;
        node.name = n.name;
        ...
        if (n.flow) {
            ...
            var flow = collectNodesInSubflow(n, sealed, toClipboard);
            node.flow = flow;
        }
        else if (n.flow === null) {
            node.flow = null;
     ...
     **/
    function createExportableNodeSet(set, exportedSubflows, exportedConfigNodes, exportAll, toClipboard) {
        var nns = [];
            ...
            if (node.type.substring(0,8) == "subflow:") {
                ...
                if (!exportedSubflows[subflowId]) {
                    ...
                    if (!subflow.sealed || exportAll) {
                        ...
                        var exportableSubflow = createExportableNodeSet(subflowSet, exportedSubflows, exportedConfigNodes, toClipboard);
                        nns = exportableSubflow.concat(nns);
                    }
                }
            }
            if (node.type != "subflow") {
                var convertedNode = RED.nodes.convertNode(node, undefined, toClipboard);
                for (var d in node._def.defaults) {
                ...
                if (!node.sealed || exportAll) {
                    var convertedSubflow = convertSubflow(node, undefined, toClipboard);
    ...
    function createCompleteNodeSet(exportCredentials, toClipboard) {
        ...
        for (i in subflows) {
            if (subflows.hasOwnProperty(i)) {
                nns.push(convertSubflow(subflows[i], exportCredentials, undefined, toClipboard));
            }
        }
        for (i in configNodes) {
            if (configNodes.hasOwnProperty(i)) {
                nns.push(convertNode(configNodes[i], exportCredentials, toClipboard));
            }
        }
        for (i=0;i<nodes.length;i++) {
            var node = nodes[i];
            nns.push(convertNode(node, exportCredentials, toClipboard));
        }
        return nns;
    }
            ...
            if (n.type !== "workspace" && n.type !== "tab" && n.type !== "subflow") {
                ...
                if (!def || def.category != "config") {
                    ...
                    if (createNewIds) {
                        ...
                        node.id = n.id;
                        if (node.z == null || (!workspaces[node.z] && !subflow_map[node.z] && !subflows[node.z])) {
                            if (createMissingWorkspace) {
                                if (missingWorkspace === null) {
                                    missingWorkspace = RED.workspaces.add(null,true);
                                    new_workspaces.push(missingWorkspace);
