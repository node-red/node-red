...
RED.sidebar = (function() {
    ...
    // We use primarySidebar.tabs.onchange to update localStorage. However that will
    ...
    function addTab(title,content,closeable,visible) {
        ...
        const targetSidebar = options.target === 'secondary' ? secondarySidebar : primarySidebar;
        ...
        if (options.visible !== false) {
            targetSidebar.tabs.addTab(knownTabs[options.id]);
        }
        if (targetSidebar.tabs.count() > 1) {
            targetSidebar.tabs.container.show()
        }
    }

    function removeTab(id) {
        if (knownTabs[id]) {
            const targetSidebar = knownTabs[id].target === 'secondary' ? secondarySidebar : primarySidebar;
            targetSidebar.tabs.removeTab(id);
            ...
            if (targetSidebar.tabs.count() <= 1) {
                targetSidebar.tabs.container.hide()
            }
        }
    }
    ...
    function setupSidebarSeparator(sidebar) {
        ...
        if (sidebar.direction === 'right') {
            ...
            scaleFactor = -1;
            controlClass = 'red-ui-sidebar-control-left';
            controlSlideDirection = 'left';
        ...
        sidebarControls.addClass(controlClass)
        ...
        separator.on("mouseenter", function() {
            if (!sidebarSeparator.dragging && !shade.is(":visible")) {
                ...
                sidebarControls.toggle("slide", { direction: controlSlideDirection }, 200);
            }
        })
    ...
    function toggleSidebar(sidebar, state) {
        if (!state) {
            ...
            // console.log(sidebar.container.width(),sidebar.tabs.container.width())
            sidebar.container.show()
            sidebar.tabs.container.width()
            setTimeout(function () {
                sidebar.tabs.resize()
            }, 100)
        }
    ...
    function showSidebar(id, skipShowSidebar) {
        ...
        if (id) {
            ...
            if (tabOptions) {
                const targetSidebar = tabOptions.target === 'secondary' ? secondarySidebar : primarySidebar;
                if (!targetSidebar.tabs.contains(id)) {
                    targetSidebar.tabs.addTab(knownTabs[id]);
                }
                targetSidebar.tabs.activateTab(id);
    ...
    function containsTab(id) {
        return primarySidebar.tabs.contains(id);
    }

    function setupSidebar(sidebar) {
        sidebar.container.addClass("red-ui-sidebar");
        sidebar.container.width(sidebar.defaultWidth);
        sidebar.separator = setupSidebarSeparator(sidebar);
        sidebar.tabs = RED.tabs.create({
            element: $('<ul></ul>').appendTo(sidebar.container),
    ...
    function init () {
        primarySidebar.container = $("#red-ui-sidebar");
        setupSidebar(primarySidebar)
        secondarySidebar.container = $("#red-ui-sidebar-left");
        setupSidebar(secondarySidebar)

        RED.actions.add("core:toggle-sidebar",function(state){
            if (state === undefined) {
                RED.menu.toggleSelected(primarySidebar.menuToggle);
            } else {
                toggleSidebar(primarySidebar, state);
            }
        });
        RED.actions.add("core:toggle-palette", function(state) {
            if (state === undefined) {
                RED.menu.toggleSelected(secondarySidebar.menuToggle);
            } else {
                toggleSidebar(secondarySidebar, state);
            }
        });


        RED.popover.tooltip(primarySidebar.separator.find(".red-ui-sidebar-control-right"),RED._("keyboard.toggleSidebar"),"core:toggle-sidebar");
