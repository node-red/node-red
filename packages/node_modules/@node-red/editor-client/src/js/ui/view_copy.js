...
RED.view = (function() {
    ...
    function canvasMouseMove() {
        ...
        if (entryCoordinates.x != -1) {
            mouse_position = [entryCoordinates.x, entryCoordinates.y]
        } else {
            mouse_position = d3.touches(this)[0]||d3.mouse(this);
        }
if(RED.view.DEBUG) { console.log(`mousemove ${JSON.stringify(mouse_position)}`)}
        if (lasso) {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            if(RED.view.DEBUG) { console.log(`mousemove - MOVING_ACTIVE ${JSON.stringify(mousePos)}`)}
    ...
    function resetMouseVars() {
        if (RED.view.DEBUG) { 
            console.warn("resetMouseVars - resetting these items: ", {
                mousedown_node,
                mousedown_group,
                mousedown_group_handle,
                mouseup_node,
                mousedown_link,
                mouse_mode,
                mousedown_port_type,
    ...
    function getPosition(canvas, event) {
        ...
        x -= canvas.offsetLeft == null ? canvas.clientLeft : canvas.offsetLeft;
        y -= canvas.offsetTop == null ? canvas.clientTop : canvas.offsetTop;

        return {x:x, y:y};
    }
    //temporary proxy for when mouse is locked
    function canvasMouseMove_locked(e) {
    
            var canvas = $(eventLayer[0])[0];
            ...
            if (entryCoordinates.x > chart.width() -65) {
                entryCoordinates.x = chart.width()-65;
            } else if (entryCoordinates.x < 0) {
                entryCoordinates.x = 0;
            }
    
            if (entryCoordinates.y > chart.height() - 85) {
                entryCoordinates.y = chart.height() - 85;
            ...
            // console.log(entryCoordinates)
        

        d3.event = e
        canvasMouseMove.call(this,e);
        //canvasMouseMove.call(document.querySelector("g.red-ui-workspace-chart-event-layer"));
    }
    function changeCallback(e) {
        var canvas = $(eventLayer[0])[0]
        // var workspaceChart = $("g.red-ui-workspace-chart-event-layer")[0];
        // var workspaceChart = $("#red-ui-workspace-chart")[0];
        if (document.pointerLockElement === canvas ||
            document.mozPointerLockElement === canvas ||
            document.webkitPointerLockElement === canvas) {

            // we've got a pointerlock for our element, add a mouselistener
            canvas.addEventListener("mousemove", canvasMouseMove_locked, false);
            // outer.on("mousemove", canvasMouseMove_locked)
            // document.addEventListener("mousemove", canvasMouseMove_locked, false);
        } else {

            // pointer lock is no longer active, remove the callback
            canvas.removeEventListener("mousemove", canvasMouseMove_locked, false);
            // document.removeEventListener("mousemove", canvasMouseMove_locked, false);
            // if(outer.off) {outer.off("mousemove", canvasMouseMove_locked)}
            // if(outer.removeEventListener) {outer.removeEventListener("mousemove", canvasMouseMove_locked)}
    ...
    function nodeMouseDown(d) {
        ...
        try {
            // workspaceChart =  $("g.red-ui-workspace-chart-event-layer")[0]
            var canvas = $(eventLayer[0])[0]
            canvas.requestPointerLock = canvas.requestPointerLock ||
                                                canvas.mozRequestPointerLock ||
                                                canvas.webkitRequestPointerLock;
            canvas.requestPointerLock()
            console.log("got pointer lock")
        } catch (error) {
            console.error(error)
        }
