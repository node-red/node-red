...
RED.sidebar.info = (function() {
    ...
    function getNodeInfo(node) {
        var i18nInfo = node.i18nInfo;
        if (i18nInfo) {
            var lc = localStorage.getItem("editor-language");
            if (i18nInfo.hasOwnProperty(lc)) {
                var text = i18nInfo[lc];
                if (text) {
                    return text;
                }
            }
        ...
        if (node === null) {
            ...
            if (node.type === "tab" || node.type === "subflow") {
                ...
                if (subflowNode && node.type !== "subflow") {
                    ...
                    helpText = (marked(getNodeInfo(subflowNode))||('<span class="red-ui-help-info-none">'+RED._("sidebar.info.none")+'</span>'));
                } else {
            ...
            if (node._def && (node._def.info || node.i18nInfo)) {
                var info = getNodeInfo(node._def);
                var textInfo = (typeof info === "function" ? info.call(node) : info);
                infoText = infoText + marked(textInfo);
            }
            if (node.info || node.i18nInfo) {
                infoText = infoText + marked(getNodeInfo(node));
            }
