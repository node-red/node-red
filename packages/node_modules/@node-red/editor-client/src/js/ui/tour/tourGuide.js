RED.tourGuide = (function() {
    ...
    function runTourStep(step, state, done) {
        ...
        prepareStep(step, state, function() {
            ...
            if (step.image) {
                // We need to wait for the image to load so its height is known before we show the popover
                $('<img>').attr({ src: `red/tours/${step.image}`, alt: getLocaleText(step.title) || "" }).appendTo(stepDescription).on('load', function() {
                    finishBuild();
                })
            }
            ...
            const stepEventListener = function() {
            ...
            // If no image, we can finish the build immediately. Otherwise, finishBuild will be called when the image has loaded
            if (!step.image) {
                finishBuild()
            }
        })
