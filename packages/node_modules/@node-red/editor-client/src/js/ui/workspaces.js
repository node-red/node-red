...
RED.workspaces = (function() {
    ...
    function addWorkspace(ws,skipHistoryEntry,targetIndex) {
        if (ws) {
            ...
            ws = {type:"tab",id:tabId,disabled: false,info:"",label:RED._('workspace.defaultName',{number:workspaceIndex}), starterNodes:[]};
    ...
    function showEditWorkspaceDialog(id) {
        ...
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        var starterNodes = $("#node-input-starternodes-container-div").treeList('selected').map(function(i) {return i.node.id});
                        if (workspace.starterNodes !== starterNodes) {
                            changes.starterNodes = workspace.starterNodes;
                            changed = true;
                            workspace.starterNodes = starterNodes;
                        }
            ...
            open: function(tray) {
                ...
                var starterNodesTitle = $('<div class="form-row node-input-title-row>'+
                    '<label style="width:300px;">Starter nodes:</label>'+
                    '</div>').appendTo(dialogForm);
                var starterNodesSelect = $('<div class="form-row node-input-starternodes-row">'+
                    '<button id="node-input-starternodes-select" class="red-ui-button" data-i18n="node-red:common.label.selectNodes">Select nodes...</button>'+
                    '</div>').appendTo(dialogForm);
                var starterNodesList = $('<div class="form-row node-input-target-row node-input-starternode-list-row" style="min-height: 100px; height: 208px;">'+
                    '<div id="node-input-starternodes-container-div"></div>'+
                    '</div>').appendTo(dialogForm);

                var starterNodes = workspace.starterNodes || [];
                var dirList = $("#node-input-starternodes-container-div").css({width: "100%", height: "100%"})
                    .treeList({multi:true})
                ...
                candidateNodes.forEach(function(n) {
                    if (n.type.indexOf("subflow:") != 0) {
                        var isChecked = starterNodes.indexOf(n.id) !== -1;
                ...
                $("#node-input-starternodes-select").on("click", function(e) {
                    e.preventDefault();
