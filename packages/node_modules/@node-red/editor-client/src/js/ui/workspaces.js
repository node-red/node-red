...
RED.workspaces = (function() {
    ...
    function addWorkspace(ws,skipHistoryEntry,targetIndex) {
        if (ws) {
            ...
            ws = {type:"tab",id:tabId,disabled: false,info:"",label:RED._('workspace.defaultName',{number:workspaceIndex}), spoutNodes:[], shutdownBehaviour:'whenReady'};
    ...
    function showEditWorkspaceDialog(id) {
        ...
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        var spoutNodes = $("#node-input-spoutnodes-container-div").treeList('selected').map(function(i) {return i.node.id});
                        if (workspace.spoutNodes !== spoutNodes) {
                            changes.spoutNodes = workspace.spoutNodes;
                            changed = true;
                            workspace.spoutNodes = spoutNodes;
                        }

                        var shutdownBehaviour = $("#node-input-shutdown-behaviour").val();
                        if (workspace.shutdownBehaviour !== shutdownBehaviour) {
                            changes.shutdownBehaviour = workspace.shutdownBehaviour;
                            changed = true;
                            workspace.shutdownBehaviour = shutdownBehaviour;
                        }
                        console.log("shutdownBehavior at end = " + workspace.shutdownBehaviour);
            ...
            open: function(tray) {
                ...
                var shutdownBehaviour = workspace.shutdownBehaviour || "whenReady";
                var spoutNodes = workspace.spoutNodes || [];

                // Hide spout node selection UI when graceful shutdown is disabled
                if (RED.settings.gracefulShutdown) {
                    var shutdownBehaviourSelect = $('<div class="form-row node-input-title-row">' +
                        '<label style="width:150px;">Shutdown Behaviour:</label>' +
                        '<select id="node-input-shutdown-behaviour">' +
                        '<option value="whenReady">When Ready</option>' +
                        '<option value="selectedNodeFirst">Selected Node First</option>'+
                        '</select>' +
                        '</div>').appendTo(dialogForm);

                    $("#node-input-shutdown-behaviour").on("change", function() {
                        if (this.value === "whenReady") {
                            $("#spoutnodes-select").hide();
                            $("#spoutnodes-container-div").hide();
                        } else if (this.value === "selectedNodeFirst") {
                            $("#spoutnodes-select").show();
                            $("#spoutnodes-container-div").show();
                        }
                    }).trigger("change");

                    var spoutNodesSelect = $('<div class="form-row node-input-spoutnodes-row" id="spoutnodes-select">' +
                        '<button id="node-input-spoutnodes-select" class="red-ui-button" data-i18n="node-red:common.label.selectNodes">Select nodes...</button>' +
                        '</div>').appendTo(dialogForm);
                    var spoutNodesList = $('<div id="spoutnodes-container-div" class="form-row node-input-target-row node-input-spoutnode-list-row" style="min-height: 100px; height: 208px;">' +
                        '<div id="node-input-spoutnodes-container-div"></div>' +
                        '</div>').appendTo(dialogForm);

                    if (shutdownBehaviour == "whenReady") {
                        $("#node-input-shutdown-behaviour").val('whenReady').trigger("change");
                    } else {
                        $("#node-input-shutdown-behaviour").val('selectedNodeFirst').trigger("change");
                    }

                    var dirList = $("#node-input-spoutnodes-container-div").css({ width: "100%", height: "100%" })
                        .treeList({ multi: true })
                    ...
                    var candidateNodes = RED.nodes.filterNodes({ z: id });
                    var items = [];
                    var nodeItemMap = {};

                    candidateNodes.forEach(function (n) {
                        if (n.type.indexOf("subflow:") != 0) {
                            var isChecked = spoutNodes.indexOf(n.id) !== -1;
                    ...
                    $("#node-input-spoutnodes-select").on("click", function (e) {
                        e.preventDefault();
                        ...
                        RED.view.selectNodes({
                            ...
                            filter: function (n) {
                                return n.type.indexOf("subflow:") != 0;
                            }
                        });
                    });
                }
