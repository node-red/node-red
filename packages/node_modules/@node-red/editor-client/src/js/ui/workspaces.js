...
RED.workspaces = (function() {
    ...
    function createWorkspaceTabs() {
        workspace_tabs = RED.tabs.create({
            ...
            onadd: function(tab) {
                ...
                const tabId = "#red-ui-tab-"+(tab.id.replace(".","-"));
                const tabElement = $(tabId);
                $('<span class="red-ui-workspace-disabled-icon"><i class="fa fa-ban"></i> </span>').prependTo(tabId + " .red-ui-tab-label");
                if (tab.disabled) {
                    tabElement.addClass('red-ui-workspace-disabled');
                }
                $('<span class="red-ui-workspace-locked-icon"><i class="fa fa-lock"></i> </span>').prependTo(tabId + " .red-ui-tab-label");
                if (tab.locked) {
                    tabElement.addClass('red-ui-workspace-locked');
                }
                tabElement.addClass('red-ui-workspace-changed');
    ...
    function init() {
        ...
        $('<div id="red-ui-workspace-chart" tabindex="0"></div>').appendTo("#red-ui-workspace");
        ...
        // Update 'changed' and 'error' badges when a workspace changes
        ...
        const updateErrorBadge = function (node) {
            const hasInvalidNode = RED.nodes.filterNodes({ z: node.z }).some((node) => !node.valid);
            const tab = $("#red-ui-tab-" + (node.z.replace(".", "-")));
            tab.toggleClass('red-ui-workspace-error', hasInvalidNode);
        };
        RED.events.on("nodes:add", updateErrorBadge);
        RED.events.on("nodes:change", updateErrorBadge);
        RED.events.on("nodes:remove", updateErrorBadge);
    ...
    return {
        ...
        refresh: function() {
            ...
            // Unlike the 'changed' badge, the 'error' badge depends on the node's validation mechanism.
            // Consequently, listening for workspace changes is not sufficient to keep error badges up to date.
            // i.e. initial loading of flows and closing node editing box without changes
            const workspaceInvalid = new Set();
            RED.nodes.eachNode((node) => {
                if (!node.valid) {
                    workspaceInvalid.add(node.z);
                }
            });
            RED.nodes.eachWorkspace(function (ws) {
                workspace_tabs.renameTab(ws.id, ws.label);
                const tab = $("#red-ui-tab-" + (ws.id.replace(".", "-")))
                tab.attr("flowname", ws.label)
                tab.toggleClass('red-ui-workspace-error', workspaceInvalid.has(ws.id));
            });
