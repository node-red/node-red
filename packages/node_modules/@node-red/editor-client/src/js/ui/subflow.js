     ...
     function buildEnvEditRow(container, ui, nameField, valueField) {
         ...
         inputCellInput.typedInput({
             types: [
                 ...
                 {
                     value:"date",
                     label:RED._("editor.inputs.date"),
                     icon:"fa fa-calendar",
                     valueLabel: function(container,value) {
                         container.css("padding",0);
                         var innerContainer = $('<div>').css({
                         ...
                         var type =
                             (ui.opts && ui.opts.hasOwnProperty("type"))
                              ? ui.opts.type : "date";
                         dateInput = $("<input/>", {
                             type: "date"
                         }).css({
                     ...
                     expand: {
                         ...
                         content: function(container) {
                             var content = $("<div>", {
                                 class: "red-ui-editor-subflow-ui-edit-panel"
                             }).appendTo(container);
                             var list = $("<ul>", {
                                 class: "red-ui-date-list"
                             }).appendTo(content);
                             var inputTypes = [
                                 "datetime-local",
                                 "date",
                             ...
                             inputTypes.forEach(function (type) {
                                 var text = RED._("editor.date-format."+type);
                                 var item = $("<li>").text(text).appendTo(list);
                                 item.on("click", function (ev) {
                                     ev.preventDefault();
                                     dateInput.attr("type", type);
                 ...
                 {
                     value: "color",
                     label: RED._("editor.inputs.color"), icon:"fa fa-paint-brush", showLabel: false,
                     valueLabel: function(container,value) {
                         container.css("padding",0);
                         var innerContainer = $('<div>').css({
                             "background":"white",
                         ...
                         var input = $('<div class="placeholder-input">').appendTo(innerContainer);
                         $('<span><i class="fa fa-paint-brush"></i></span>').appendTo(input);                         
                     }
                 },
                 {
                     value:"none",
    ...
    function buildEnvUIRow(row, tenv, ui, node) {
        ...
        switch(ui.type) {
            ...
            case "spinner":
                ...
                break;
            case "date":
                var type = ui.opts.hasOwnProperty("type") ? ui.opts.type : "date";
                input = $("<input>", {
                    type: type
                    ...
                    if (all || !item.parent || (item.parent.value !== value || item.parent.type !== type)) {
                        ...
                        if (item.ui) {
                            ...
                            switch (ui.type) {
                                ...
                                case "date":
                                    if ($.isEmptyObject(ui.opts)) {
                                        // This is the default date config.
                                        // Delete as it will be applied automatically
                                        delete ui.opts
                                    }
                                    break;
