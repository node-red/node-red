...
RED.subflow = (function() {
    ...
    var _subflowModulePaneTemplate = '<form class="dialog-form form-horizontal" autocomplete="off">'+
        ...
        '</div>'+
        '<div class="form-row" id="subflow-encoding-div">'+
            '<label data-i18n="[append]editor:subflow.encoding"></label>'+
            '<select style="width: calc(100% - 110px)" id="subflow-input-module-encoding"></select>'+
        '</div>'+
        '<div class="form-row" id="subflow-encodeKey-div">'+
            '<label for="subflow-input-module-encodekey" data-i18n="[append]editor:subflow.encodekey"></label>'+
            '<input style="width: calc(100% - 110px)" type="text" id="subflow-input-module-encodekey" data-i18n="[placeholder]editor:subflow.encodekeyPlaceholder"></input>'+
        '</div>'+
        '<div id="subflow-download-button-div">'+
            '<a id="subflow-download-button" href="#" class="red-ui-button primary" data-i18n="subflow.download"></a>'+
        '</div>'+
    '</form>';
    ...
    function checkRequiredMeta() {
        var meta = exportSubflowModuleProperties(null);
        if (meta.module && meta.type && meta.version &&
            (meta.module !== "") &&
            (meta.type !== "") &&
            (meta.version !== "")) {
            return meta;
    ...
    function downloadSubflowNPM(meta, json, done) {
        var file = meta.module +"-" +meta.version +".tgz";
        $.ajax({
            url: "/module/create/subflow",
            type: "POST",
            data: JSON.stringify({
                meta: meta,
                flow: json
            ...
            RED.utils.downloadData(file, data, true);
            done();
        }).fail(function (xhr, stat, err) {
            RED.notify("failed to create NPM module");
            done();
        });
    }

    function requiresKey(enc) {
        if (enc === "AES") {
            return true;
        }
        if (RED.settings.encodeSubflow) {
            var esf = RED.settings.encodeSubflow;
            var methods = esf.methods;
            if (methods) {
                for (var i = 0; i < methods.length; i++) {
                    var method = methods[i];
                    if (method.name === enc) {
                        return (method.requiresKey === true);
                    }
                }
            }
        }
    ...
    function checkDownload() {
        var props = ["module", "type", "version"];
        var ok = true;
        for (var i = 0; i < props.length; i++) {
            var id = "#subflow-input-module-"+props[i];
            var val = $(id).val();
            if (!val || (val === "")) {
                $("#subflow-download-button").addClass("disabled");
                return;
            }
        }
        $("#subflow-download-button").removeClass("disabled");
    }
    ...
    function buildModuleForm(container, node) {
        ...
        var encoding = $("#subflow-input-module-encoding");
        var encs = ["none", "AES"];
        encs.forEach(function (enc) {
            var name = (enc === "none") ? RED._("editor:subflow.noEncoding") : enc;
            $("<option/>", {
                value: enc
            }).text(name).appendTo(encoding);
        });
        encoding.on("change", function(ev) {
            var enc = encoding.val();
            if (!requiresKey(enc)) {
                $("#subflow-encodeKey-div").hide();
            }
            else {
                $("#subflow-encodeKey-div").show();
            }
        });
        encoding.val(moduleProps["encoding"] || "none");
        encoding.change();

        var exp = $("#subflow-download-button");
        exp.css({
            "float": "right",
            "margin-right": "10px"
        });
        exp.on("click", function (ev) {
            ev.preventDefault();
            var meta = checkRequiredMeta();
            if (meta) {
                var json = createSubflowJSON();
                if (json) {
                    downloadSubflowNPM(meta, json, function () {
                        exp.blur();
                    });
                }
                else {
                    RED.notify(RED._("editor:subflow.errors.failedNPMCreation"));
                }
            }
            else {
                RED.notify(RED._("editor:subflow.errors.notFilled"));
            }
        });
        if (RED.settings.encodeSubflow) {
            var esf = RED.settings.encodeSubflow;
            ...
            if (esf["default"] && !moduleProps["encoding"]) {
                var val = esf["default"];
                encoding.val(val);
            }
        }
        var sfm = RED.settings.subflowModule;
        var enableDownload = true;
        if (sfm && !sfm.allowDownload) {
            enableDownload = false;
        }
        if (enableDownload) {
            if (sfm &&
                sfm.hasOwnProperty("allowEncoding") &&
                !sfm.allowEncoding) {
                $("#subflow-encoding-div").hide();
                $("#subflow-encodeKey-div").hide();
            }
            var props = ["module", "type", "version"];
            props.forEach(function (prop) {
                var id = "#subflow-input-module-"+prop;
            ...
            $("#subflow-download-button-div").hide();
        }
    }
