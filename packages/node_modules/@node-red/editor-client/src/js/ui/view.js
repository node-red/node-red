...
RED.view = (function() {
    ...
    function init() {
        ...
        chart.droppable({
            ...
            drop: function( event, ui ) {
                ...
                try {
                    ...
                    const showLabelUserSetting = RED.settings.get('editor.view.view-node-show-label', true);
                    const showLabelNodeDefinition = Object.prototype.hasOwnProperty.call(nn._def, "showLabel") ? !!nn._def.showLabel : true;

                    if (!showLabelUserSetting && !Object.prototype.hasOwnProperty.call(nn, "l") && showLabelNodeDefinition) {
                        // If the user wants to hide the label of newly added nodes, the "l" property must be added to the node to override its state.
                        // If the node definition already hides the label, the state should not be overridden, as it is identical.
                        nn.l = false;
                    }

                    const showLabel = Object.prototype.hasOwnProperty.call(nn, "l") ? !!nn.l : showLabelNodeDefinition;
        ...
        $("#red-ui-view-zoom-out").attr('aria-label', RED._('actions.zoom-out')).on("click", function() { zoomOut(); });
        RED.popover.tooltip($("#red-ui-view-zoom-out"),RED._('actions.zoom-out'),'core:zoom-out');
        $("#red-ui-view-zoom-zero").attr('aria-label', RED._('actions.zoom-reset')).on("click", zoomZero);
        RED.popover.tooltip($("#red-ui-view-zoom-zero"),RED._('actions.zoom-reset'),'core:zoom-reset');
        $("#red-ui-view-zoom-in").attr('aria-label', RED._('actions.zoom-in')).on("click", function() { zoomIn(); });
        RED.popover.tooltip($("#red-ui-view-zoom-in"),RED._('actions.zoom-in'),'core:zoom-in');
        $("#red-ui-view-zoom-fit").attr('aria-label', RED._('actions.zoom-fit')).on("click", zoomToFitAll);
    ...
    function showTooltip(x,y,content,direction) {
        const tooltip = eventLayer.append("g")
        ...
        let labelHeight = innerPadding;
        const labelHeights = [];
        let lineHeight = 0;
        lines.forEach(function(l,i) {
            const labelDimensions = calculateTextDimensions(l||"&nbsp;", "red-ui-flow-port-tooltip-label");
            labelWidth = Math.max(labelWidth,labelDimensions[0] + 8);
        ...
        const arrowBaseLength = 7;
        labelHeight -= cornerRadius * 2;

        const labelWidth1 = (labelWidth/2) - arrowBaseLength - cornerRadius;
        const labelWidth2 = labelWidth - (2 * cornerRadius);

        const labelHeight1 = (labelHeight/2) - arrowBaseLength;
        const labelHeight2 = labelHeight
        ...
        if (direction === "left") {
            path = `M0 0 l -${arrowBaseLength} -${arrowBaseLength} v -${labelHeight1} q 0 -4 -4 -4 h -${labelWidth} q -4 0 -4 4 v ${labelHeight2} q 0 4 4 4 h ${labelWidth} q 4 0 4 -4 v -${labelHeight1} l ${arrowBaseLength} -${arrowBaseLength}`;
            lx = -arrowBaseLength - innerPadding;
            anchor = "end";
    ...
    function nodeMouseOver(d) {
        ...
        if (mouse_mode === 0 || mouse_mode === RED.state.SELECTING_NODE) {
            ...
            clearTimeout(portLabelHoverTimeout);
            const showLabelNodeDefinition = Object.prototype.hasOwnProperty.call(d._def, "showLabel") ? !!d._def.showLabel : true;
            const showLabel = Object.prototype.hasOwnProperty.call(d, "l") ? !!d.l : showLabelNodeDefinition;
            if (!showLabel) {
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            const showLabelNodeDefinition = Object.prototype.hasOwnProperty.call(node._def, "showLabel") ? !!node._def.showLabel : true;
            const showLabel = Object.prototype.hasOwnProperty.call(node, "l") ? !!node.l : showLabelNodeDefinition;
            const hideLabel = !showLabel;
