...
RED.view = (function() {
    ...
    const nodeTimers = {};
    const DEFAULT_STATUS_DURATION = 5000; // 5s
    const MIN_STATUS_DURATION = 100;      // 100ms
    function redrawStatus(d,nodeEl) {
        if (nodeTimers[d.id]) {
            // Cancels the previous ephemeral status
            clearTimeout(nodeTimers[d.id]);
            delete nodeTimers[d.id];
        }
            ...
            if (!showStatus || !d.status || d.d === true) {
                ...
                if (d.status.ephemeral === true) {
                    if (d.status.duration) {
                        ...
                        // Set a default timeout for ephemeral status
                        duration = DEFAULT_STATUS_DURATION;
                    }
                }
                if (duration) {
                    d.status = d.status._retainedStatus || {};
                    d.dirtyStatus = true;
                    // Set a timeout to restore the previous status or clear it if no status
                    nodeTimers[d.id] = setTimeout(() => redrawStatus(d, nodeEl), duration);
                }
            }
