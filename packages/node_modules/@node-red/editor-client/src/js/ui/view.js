...
RED.view = (function() {
    ...
    function init() {
        ...
        chart.on("wheel", function(evt) {
            ...
            if (evt.ctrlKey || evt.altKey || spacebarPressed) {
                ...
                if (evt.ctrlKey && !evt.altKey && !spacebarPressed) {
                    // Use the zoom animator's delta calculation for trackpad
                    ...
                    // Start gesture tracking if not already active
                    if (!RED.view.zoomAnimator.isGestureActive()) {
                        RED.view.zoomAnimator.startGesture(cursorPos, scaleFactor);
                    }
                    
                    // Update gesture with new scale, maintaining focal point
                    RED.view.zoomAnimator.updateGesture(newScale);
                    zoomView(newScale, RED.view.zoomAnimator.getGestureFocalPoint()); // Direct call, no animation
                    
                    // Reset gesture timeout - end gesture when no more events come in
                    ...
                    }, 100); // 100ms timeout to detect gesture end
                } else {
                    // Regular Alt+scroll or Space+scroll - use animated zoom
                    if (delta > 0) { zoomOut(cursorPos); }
                    else if (delta < 0) { zoomIn(cursorPos); }
                }
            } else {
                // Regular scroll - track velocity and apply momentum
                handleScroll();
    ...
    function zoomView(factor, focalPoint) {
        ...
        // Use focal point if provided (e.g., cursor position), otherwise use viewport center
        var center;
        if (focalPoint) {
        ...
        // Calculate where the focal point will be after zoom
        var newCenter;
        if (focalPoint) {
            // Keep the focal point at the same screen position
            newCenter = [(scrollPos[0] + focalPoint[0])/scaleFactor, (scrollPos[1] + focalPoint[1])/scaleFactor];
        } else {
            newCenter = [(scrollPos[0] + screenSize[0]/2)/scaleFactor,(scrollPos[1] + screenSize[1]/2)/scaleFactor];
        }
