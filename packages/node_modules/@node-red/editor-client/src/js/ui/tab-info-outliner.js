RED.sidebar.info.outliner = (function() {
    ...
    function addBadges(node, div) {
        const badges = $('<div>', { class: "red-ui-info-outline-item-badges" }).appendTo(div);
        const changeBadgeContainer = $('<svg class="red-ui-flow-node-changed" width="10" height="10" viewBox="-1 -1 12 12"></svg>').appendTo(badges)
        ...
        const errorBadgeContainer = $('<svg class="red-ui-flow-node-error" width="10" height="10"></svg>').appendTo(badges)
        const errorBadge = document.createElementNS("http://www.w3.org/2000/svg", "path");
        errorBadge.setAttribute("d", "M 0,9 l 10,0 -5,-8 z");
        errorBadgeContainer.append(errorBadge);

        const docsBadgeContainer = $('<svg class="red-ui-flow-node-docs" width="10" height="10"></svg>').appendTo(badges)
        ...
        docsBadgeContainer.append(docsBadge);
    }

    function updateBadges(node) {
        const existingObject = objects[node.id];

        const flowChanged = node.type === "tab" ? (node.contentsChanged || node.changed || node.added) : false;
        const subflowChanged = node.type === "subflow" ? (node.contentsChanged || node.changed || node.added) : false;
        const nodeChanged = node.type !== "tab" && node.type !== "subflow" ? (node.changed || node.moved) : false;
        if (flowChanged || subflowChanged || nodeChanged) {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-changed").show()
        } else {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-changed").hide()
        }

        const flowInvalid = node.type === "tab" ? node.contentsInvalid : false;
        const subflowInvalid = node.type === "subflow" ? node.contentsInvalid : false;
        const nodeInvalid = node.type !== "tab" && node.type !== "subflow" ? node.valid === false : false;
        if (flowInvalid || subflowInvalid || nodeInvalid) {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-error").show()
        } else {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-error").hide()
        }

        let hasDocs = !!node.info;
        if (!hasDocs && node.type !== "comment" && node._def && node._def.info) {
            const info = node._def.info
            hasDocs = !!(typeof info === "function" ? info.call(node) : info);
        }
        if (hasDocs) {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-docs").show()
        } else {
            existingObject.element.find(".red-ui-info-outline-item-badges .red-ui-flow-node-docs").hide()
        }
    ...
    function addControls(n,div) {
        ...
        if (n.type === "tab") {
            var hideLabel = function () {
                var willHide = !div.hasClass("red-ui-info-outline-item-hidden");
                return RED._("sidebar.info." + (willHide ? "hideFlow" : "showFlow"));
            };
            var toggleVisibleButton = $('<button type="button" class="red-ui-info-outline-item-control-hide red-ui-button red-ui-button-small"><i class="fa fa-eye"></i><i class="fa fa-eye-slash"></i></button>').appendTo(controls).attr('aria-label', hideLabel()).on("click",function(evt) {
                evt.preventDefault();
                evt.stopPropagation();
                ...
                toggleVisibleButton.attr('aria-label', hideLabel());
            });
            RED.popover.tooltip(toggleVisibleButton, hideLabel);
        }
        if (n.type !== 'subflow') {
            ...
            var toggleButton = $('<button type="button" class="red-ui-info-outline-item-control-disable red-ui-button red-ui-button-small"><i class="fa fa-circle-thin"></i><i class="fa fa-ban"></i></button>').appendTo(controls).attr('aria-label', disableLabel()).on("click",function(evt) {
                evt.preventDefault();
                evt.stopPropagation();
                if (n.type === 'tab') {
                    if (n.disabled) {
                ...
                toggleButton.attr('aria-label', disableLabel());
            });
            RED.popover.tooltip(toggleButton, disableLabel);
        }
        if (n.type === 'tab') {
            var lockLabel = function () {
                return RED._("common.label." + (n.locked ? "unlock" : "lock"));
            };
            var lockToggleButton = $('<button type="button" class="red-ui-info-outline-item-control-lock red-ui-button red-ui-button-small"><i class="fa fa-unlock-alt"></i><i class="fa fa-lock"></i></button>').appendTo(controls).attr('aria-label', lockLabel()).on("click",function(evt) {
                evt.preventDefault();
                evt.stopPropagation();
                if (n.locked) {
                ...
                lockToggleButton.attr('aria-label', lockLabel());
            })
            RED.popover.tooltip(lockToggleButton, lockLabel);
    ...
    function build() {
        ...
        RED.events.on("flows:validate", updateBadges)
    ...
    function init() {
        ...
        RED.sidebar.addTab({
            id: "explorer",
            // target: "secondary",
            label: RED._("sidebar.explorer.label"),
            name: RED._("sidebar.explorer.name"),
