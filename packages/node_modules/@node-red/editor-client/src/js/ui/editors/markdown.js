...
(function() {
    ...
    function buildLocaleSelector(container, editor, editCon, i18nTexts, locale) {
        editCon.css({
            height: "calc(100% - 25px)"
        });
        var locs = $("<div/>", {
            class: "node-text-editor-locales-row"
        }).css({
            position: "relative",
            top: "3px",
            float: "right"
        ...
        if (locale) {
            editor.currentLocale = locale;
        }
        else if (!editor.currentLocale) {
            editor.currentLocale = "en-US";
        }
        editor.i18nTexts = i18nTexts ? i18nTexts : {};
        editor.localeSelector = locales;

        locales.val(editor.currentLocale);
        locales.on("change", function (e) {
            e.preventDefault();
            var lc = locales.val();
            changeLocale(editor, lc, true);
        });
    }

    function saveText(editor, lc) {
        var i18nTexts = editor.i18nTexts;
        if (!i18nTexts) {
            i18nTexts = {};
            editor.i18nTexts = i18nTexts;
        }
        i18nTexts[lc] = editor.getValue();
    }

    function getTextInfo(editor, lc) {
        var i18nTexts = editor.i18nTexts;
        if (i18nTexts) {
            if (i18nTexts.hasOwnProperty(lc)) {
                return i18nTexts[lc];
            }
        }
        return "";
    ...
    function setI18nTexts(editor, texts, locale) {
        var info = {};
        Object.keys(texts).forEach(function (lc) {
            info[lc] = texts[lc];
        });
        editor.i18nTexts = info;
        if (locale) {
            var text = info[locale] ? info[locale] : "";
            editor.setValue(text, -1);
            editor.currentLocale = locale;
            if(editor.localeSelector) {
                editor.localeSelector.val(locale);
            }
        }
    }

    function getI18nTexts(editor) {
        var texts = {};
        saveText(editor, editor.currentLocale);
        var localesList = RED.settings.theme("languages");
        localesList.forEach(function (lc) {
            texts[lc] = getTextInfo(editor, lc);
        });
        return texts;
    }

    function getText(editor, lc) {
        var texts = definition.getI18nTexts(editor);
        if (!lc) {
            lc = editor.getLocale(editor);
        }
        if (texts.hasOwnProperty(lc)) {
            return texts[lc];
        }
        return (texts["en-US"] || "");
    }

    function changeLocale(editor, lc) {
        var text = getText(editor, lc);
        editor.setValue(text, -1);
        editor.currentLocale = lc;
        if (editor.localeSelector) {
            editor.localeSelector.val(lc);
        }
    }

    function setLocale(editor, lc) {
        editor.currentLocale = lc;
    }

    function getLocale(editor) {
        return editor.currentLocale;
    }
    ...
    var definition = {
                    ...
                    },
                    {
                        ...
                        click: function() {
                            onComplete(expressionEditor.getValue(),
                                       expressionEditor.getCursorPosition(),
                                       getI18nTexts(expressionEditor),
                                       getLocale(expressionEditor));
                            RED.tray.close();
                        }
                    }
                ],
        ...
        buildLocaleSelector: buildLocaleSelector,
        setI18nTexts: setI18nTexts,
        getI18nTexts: getI18nTexts,
        getText: getText,
