            ...
            for (let exlibIdx = 0; exlibIdx < libsToLoad.length; exlibIdx++) {
                ...
                if(knownModule) {
                    ...
                    if( def ) {
                        ...
                        if(!preloadOnly) loadedLibs.JS[libName] = monaco.languages.typescript.javascriptDefaults.addExtraLib(def, `file://node_modules/@types/${libmodule}/${libName}/index.d.ts`);
                    } else {
        ...
        ed.destroy = function destroy() {
            try {
                ...
                if(Object.keys(loadedLibs.JS).length) {
                    ...
                    //console.log("disposing " + entries.length + " loadedLibs.JS")
                ...
                if(Object.keys(loadedLibs.TS).length) {
                    ...
                    //console.log("disposing " + entries.length + " loadedLibs.TS")
                    for (let i = 0; i < entries.length; i++) {
                        try {
                            const entry = entries[i];
                            let name = entry[0];
