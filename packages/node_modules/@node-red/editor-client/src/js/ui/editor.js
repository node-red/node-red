...
RED.editor = (function() {
    ...
    const workspaceInvalidMap = {};

    /**
     * Check if a flow/subflow should update its contentsInvalid flag
     * @param {string} z
     */
    function checkTabState(z) {
        const workspace = RED.nodes.workspace(z) || RED.nodes.subflow(z);
        const invalid = Math.max(0, workspaceInvalidMap[z]?.size) > 0;
        if (workspace && workspace.contentsInvalid !== invalid) {
            workspace.contentsInvalid = invalid;
            if (workspace.type === "tab") {
                RED.events.emit("flows:validate", workspace);
            } else {
                RED.events.emit("subflows:validate", workspace);
            }
        } else if (!workspace) {
            delete workspaceInvalidMap[z];
        }
    }

    /**
     * Update our records if a node is invalid or not
     * @param {string} z
     * @param {string} id
     * @param {boolean} valid
     */
    function markNodeInvalid(z, id, valid) {
        if (!z) {
            // Exclude config node
            return;
        }
        if (valid) {
            workspaceInvalidMap[z]?.delete(id);
        } else if (!workspaceInvalidMap[z]) {
            workspaceInvalidMap[z] = new Set();
            workspaceInvalidMap[z].add(id);
        } else {
            workspaceInvalidMap[z].add(id);
        }
        checkTabState(z);
    ...
    function validateNode(node) {
        ...
        markNodeInvalid(node.z, node.id, node.valid);
        if (oldValue !== node.valid) {
            RED.events.emit("nodes:validate", node);
        }
    ...
    function getNodeCredentials(type, id, done) {
        ...
        var intialTimeout = setTimeout(function() {
            timeoutNotification = RED.notify($('<p data-i18n="[prepend]editor.loadCredentials">  <img src="red/images/spin.svg" alt=""/></p>').i18n(),{fixed: true})
        },800);
