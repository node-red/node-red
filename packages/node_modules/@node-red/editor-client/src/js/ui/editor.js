...
RED.editor = (function() {
    ...
    function isAllEmpty(info) {
        var result = true;
        Object.values(info).forEach(function (v) {
            if (v !== "") {
                result = false;
            }
        });
    ...
    function isSameI18nInfo(info0, info1) {
        if (info0 === info1) {
            return true;
        }
        if ((!info0 && isAllEmpty(info1)) ||
            (!info1 && isAllEmpty(info0))) {
            return true;
        }
        if (!info0 || !info1) {
            return false;
        }
        var keys0 = Object.keys(info0);
        var keys1 = Object.keys(info1);
        if (keys0.length === keys1.length) {
            for(var i = 0; i < keys0.length; i++) {
                var key = keys0[i];
                if (!info1.hasOwnProperty(key) ||
                    (info0[key] !== info1[key])
                   ) {
                    return false;
                }
    ...
    function buildDescriptionForm(container,node) {
        ...
        var i18nInfo = null;
        if (node.i18nInfo) {
            i18nInfo = cloneObj(node.i18nInfo);
            if (node.info) {
                i18nInfo["en-US"] = node.info;
            }
        }
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (nodeInfoEditor) {
                            var i18nTexts = customEditTypes['_markdown'].getI18nTexts(nodeInfoEditor);
                            var newInfo = "";
                            if (i18nTexts.hasOwnProperty("en-US")) {
                                newInfo = i18nTexts["en-US"];
                                delete i18nTexts["en-US"];
                            }
                            if (!!oldInfo) {
                            ...
                            var oldI18nTexts = node.i18nInfo;
                            if (!isSameI18nInfo(oldI18nTexts, i18nTexts)) {
                                changed = true;
                                changes.i18nInfo = oldI18nTexts;
                                node.i18nInfo = i18nTexts;
                            }
                        }
            ...
            },
            {
                ...
                click: function() {
                    ...
                    if (nodeInfoEditor) {
                        ...
                        if (i18nTexts) {
                            editing_config_node.i18nInfo = i18nTexts;
                        }
                    }
    ...
    function createEditor(options) {
        var el0 = options.element || $("#"+options.id)[0];
        var toolbarRow = $("<div>").appendTo(el0);
        var el = $("<div>").appendTo(el0).addClass("red-ui-editor-text-container")[0];
        var localeRow = $("<div/>").appendTo(el0);
        ...
        if (options.mode === 'ace/mode/markdown') {
            ...
            if (options.expandable !== false) {
                ...
                var md = customEditTypes['_markdown'];
                expandButton.on("click", function(e) {
                    e.preventDefault();
                    var value = editor.getValue();
                    var i18nTexts = md.getI18nTexts(editor);
                    var locale = md.getLocale(editor);
                    RED.editor.editMarkdown({
                        ...
                        i18nTexts: i18nTexts,
                        locale: locale,
                        complete: function(v, cursor, i18nTexts, locale) {
                            md.setI18nTexts(editor, i18nTexts, locale);
            ...
            var md = customEditTypes['_markdown'];
            if (options.i18n) {
                md.buildLocaleSelector(localeRow, editor, $(el0),
                                       options.i18nTexts,
                                       options.locale);
            }
        }
    ...
    return {
        ...
        getTypeEditor: function(type) {
            return customEditTypes[type];
        }
    }
})();
