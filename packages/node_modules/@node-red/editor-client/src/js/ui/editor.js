...
RED.editor = (function() {
    ...
    /**
     * Create type selectio interface for typedInput
     * @param row - container row
     * @param ui - UI definition
     */
    function buildTypeSelector(row, ui) {
        var inputTypes = ui.inputTypes || {
            str: true,
            num: true,
            bool: true,
            json: true,
        ...
        types.forEach(function (type) {
            ...
            check.on("change", function (e) {
                inputTypes[type] = check.prop("checked");
            });

            check.prop("checked",
                       (inputTypes.hasOwnProperty(type) &&
                        inputTypes[type]));
    ...
    /**
     * Create menu definition interface
     * @param row - container row
     * @param ui - UI definition
     */
    function buildMenuItems(row, ui) {
        var items = ui.menu || [];
        ui.menu = items;
        var menu = $("<ol/>", {
            class: "ui-menu-item-list"
        }).appendTo(row);
        menu
            .css("min-height", "120px")
            .editableList({
                addItem: function (row, i, opt) {
                    var row = $("<div/>").appendTo(row);
                    var field = $("<input/>", {
                        class: "ui-menu-item",
                        type: "text"
                    }).css({
                    ...
                    var labels = {};
                    if (opt) {
                        labels = opt.labels ? opt.labels : {};
                    }
                    field.data("labels", labels);
                    var label = lookupLabel(labels, "", currentLocale);
                    field.val(label);
                    field.change(function () {
                        var label = field.val();
                        var labels = field.data("labels");
                        labels = labels ? labels : {};
                        labels[currentLocale] = label;
                        field.data("labels", labels);
                    });
                },
        ...
        if (items) {
            items.forEach(function (item) {
                menu.editableList("addItem", item);
            });
        }
    } // buildMenuItems

    /**
     * Create spinner definition interface
     * @param row - container row
     * @param ui - UI definition
     */
    function buildSpinner(row, ui) {
        $("<label/>").css({
            "margin-left": "30px",
            "width": "initial"
        }).text(RED._("editor.spinner.min")).appendTo(row);
        var min = $("<input/>", {
            type: "number"
        }).css({
            "margin-left": "10px",
        ...
        $("<label/>").css({
            "margin-left": "13px",
            "width": "initial"
        }).text(RED._("editor.spinner.max")).appendTo(row);
        var max = $("<input/>", {
            type: "number"
        }).css({
            "margin-left": "10px",
     ...
     * @param icon - icon spec
     */
    function showFaIcon(elem, icon) {
        var newPath = RED.utils.separateIconPath(icon);
        if (newPath) {
     ...
     */
    function buildUIEdit(container, env, nameField, valueField) {
        var name = env.name;
        var ui = env.ui;
        // 1st: expander, variable, and defeault value
        var row0 = $("<div/>").css({
            "margin-top": "5px"
        }).appendTo(container);
        $("<label/>").css({
        ...
        $("<label/>").css({
            width: "40px",
            "margin-left": "6px"
        }).text(RED._("editor.icon")).appendTo(row0);
        $("<label/>").css({
            width: "calc(70% - 56px)",
            "margin-left": "6px"
        }).text(RED._("editor.label")).appendTo(row0);
        $("<label/>").css({
            width: "calc(30% - 30px)",
            "margin-left": "6px"
        }).text(RED._("editor.inputType")).appendTo(row0);

        // 2nd: UI checkbox, icon, label, and input type
        var row1 = $("<div/>").appendTo(container);
        var hasUICheck = $("<input/>", {
            type: "checkbox"
        }).css({
            width: "27px"
        }).appendTo(row1);
        ...
        var faIcon = $("<i/>").appendTo(iconButton);
        var label = $("<input/>", {
            type: "text",
            class: "env-var-label"
        }).css({
        ...
        label.data("labels", (ui.labels ? ui.labels : {}));
        var uiSelect = $("<select/>", {
            type: "text",
            class: "env-var-type"
        }).css({
        ...
        ["input", "select", "checkbox", "spinner", "hidden"].forEach(function (item) {
            var text = RED._("editor.menu."+item);
            $("<option/>", {
                value: item
            }).text(text).appendTo(uiSelect);
        });

        // 3rd: typedInput options
        var rowInput = $("<div/>").css({
            "margin-top": "10px"
        }).appendTo(container);
        buildTypeSelector(rowInput, ui);
        ...
        buildMenuItems(rowMenu, ui);

        // 5th: spinner options
        var rowSpinner = $("<div/>").css({
            "margin-top": "10px"
        }).appendTo(container);
        buildSpinner(rowSpinner, ui);

        // actions
        hasUICheck.change(function(e) {
            ui.hasUI = hasUICheck.prop("checked");
        });
        iconButton.on("click", function(e) {
            e.preventDefault();
            var icon = ui.icon || "";
            var iconPath = (icon ? RED.utils.separateIconPath(icon) : {});
            showIconPicker(row1, null, iconPath, true, function (newIcon) {
                var path = newIcon || "";
                showFaIcon(faIcon, path);
                ui.icon = path;
            });
        });
        label.change(function (e) {
            var labels = (ui.hasOwnProperty("labels") ? ui.labels : null) || {};
            ui.labels = labels;
            ui.labels[currentLocale] = label.val();
            label.data("labels", labels);
        });
        if (nameField) {
            nameField.change(function (e) {
                label.attr("placeholder", nameField.val());
            });
        }
        uiSelect.change(function (e) {
            var type = uiSelect.val();
            ui.type = type;
            rowInput.hide();
            rowMenu.hide();
            rowSpinner.hide();
            if (type === "input") {
                ...
                valueField.typedInput("type", "noval");
                rowMenu.show();
            }
        ...
        // default values
        ui.icon = ui.hasOwnProperty("icon") ? ui.icon : "";
        ui.labels = ui.hasOwnProperty("labels") ? ui.labels : {};
        ui.type = ui.hasOwnProperty("type") ? ui.type : "input";

        hasUICheck.prop("checked", ui.hasUI);
        label.val(lookupLabel(ui.labels, "", currentLocale));
        label.attr("placeholder", name);
        uiSelect.val(ui.type);

        hasUICheck.change();
        showFaIcon(faIcon, ui.icon);
        label.change();
        uiSelect.change();
    }
    // buildUIEdit
    ...
    /**
     * Find n-th value field of env var
     * @param container - container list
     * @param n - position
     */
    function findValueField(container, n) {
        var list = container.editableList("items");
        var item = (list.length > n) ? list[n] : null;
        if (item) {
            return item.find(".node-input-env-value");
        }
        return null;
    }

    function lookupType(tenv, ienv) {
        if (ienv && ienv.hasOwnProperty("type")) {
            return ienv.type;
        }
        if (tenv && tenv.hasOwnProperty("type")) {
            return tenv.type;
        }
        return undefined;
    }

    function lookupValue(tenv, ienv) {
        if (ienv && ienv.hasOwnProperty("value")) {
            return ienv.value;
        }
        if (tenv && tenv.hasOwnProperty("value")) {
            return tenv.value;
        }
        return undefined;
    }
     ...
     * @param uiCon - container for UI
     * @param envCon - container for env var
     * @param tenvs - env var definitions of template
     * @param ienvs - env var definitions of instance
     */
    function buildEnvUI(uiCon, envCon, tenvs, ienvs) {
        uiCon.empty();
        for (var i = 0; i < tenvs.length; i++) {
            var tenv = tenvs[i];
            var ienv = ienvs ? ienvs[i] : null;
            var ui = tenv.ui;
            if (ui && ui.hasUI) {
                var labels = ui.labels || {};
                var label = lookupLabel(labels, "", currentLocale);
                label = (label === "") ? tenv.name : label;
                var row = $("<div/>", {
                    class: "form-row"
                }).appendTo(uiCon);
                var type = ui.type ? ui.type : "input";
                if (type === "checkbox") {
                    var padding = $("<label/>", {
                        style: "width: 25%;"
                    }).html("").appendTo(row);
                    var id = "node-red-input-checkbox"+elementID;
                    elementID++;
                    ...
                    $("<label/>", {
                        ...
                        width: "25%"
                    }).html(icon+label).appendTo(row);
                    if (envCon) {
                        var value = lookupValue(tenv, ienv);
                        var val = ((value === "true") || (value === true));
                        valueField.prop("checked", val);
                        valueField.change((function (pos, field) {
                            return function (e) {
                                var new_val = field.prop("checked") ? "true" : "false";
                                var input = findValueField(envCon, pos);
                                input.typedInput("type", "bool");
                                input.typedInput("value", new_val);
                            };
                        })(i, valueField));
                    }
                }
                    ...
                    if (type === "input") {
                        ...
                        var allTypes = ui.inputTypes ? Object.keys(ui.inputTypes) : [];
                        var types = allTypes.filter(function (name) {
                            return ui.inputTypes[name];
                        });
                        if (types.length === 0) {
                        ...
                        valueField.typedInput("width", "65%");
                        if (envCon) {
                            var type = lookupType(tenv, ienv);
                            var value = lookupValue(tenv, ienv);
                            ...
                            valueField.change(
                                (function (pos, field) {
                                    return function (v, t) {
                                        var type = field.typedInput("type");
                                        var value = field.typedInput("value");
                                        var input = findValueField(envCon, pos);
                        ...
                        items.forEach(function (elem) {
                            ...
                            var en = lookupLabel(labels, "", "en-US");
                            var value = lookupLabel(labels, en, currentLocale);
                            if (initVal == null) {
                                initVal = en;
                            }
                            $("<option>", {
                        ...
                        if (ui.spinner) {
                            if (ui.spinner.min) {
                                valueField.spinner({min: ui.spinner.min});
                                valueField.spinner("value", ui.spinner.min);
                            }
                            if (ui.spinner.max) {
                                valueField.spinner({max: ui.spinner.max});
                            }
                       }
                        if (envCon) {
                            var value = lookupValue(tenv, ienv);
                            if (value && (value !== "")) {
                                valueField.spinner("value", value);
                            }
                            valueField.spinner({
                                change: (function (pos, field) {
                                    return function (e) {
                                        var input = findValueField(envCon, pos);
                                        var value = field.val();
    ...
    /**
     * Change Current Label Locale for Env Var UI
     * @param items - list of UI definition items
     * @param locale - new locale
     */
    function changeLabelLocale(items, locale) {
        if (items) {
            items.each(function (i, item) {
                var entry = $(this);
                var labelField = entry.find(".env-var-label");
                var labels = labelField.data("labels");
                var label = lookupLabel(labels, labelField.val(), locale);
                labelField.val(label);

                var menu = entry.find(".ui-menu-item-list").editableList("items");
                menu.each(function (i, e) {
                    var menuItem = $(this).find(".ui-menu-item");
                    var values = menuItem.data("labels");
                    var value = lookupLabel(values, menuItem.val(), locale);
                    menuItem.val(value);
                });
            });
        }
     ...
     * @param controlCon - container for controller
     * @param envCon - container for env var
     */
    function buildEnvControl(controlCon, envCon) {
        var localesDiv = $("#subflow-input-env-locale");
        ...
        locales.on("change", function() {
            ...
            changeLabelLocale(items, currentLocale);
        });
        restoreLocaleTip = RED.popover.tooltip(locales,RED._("editor.locale"));

        var buttonDiv = $("#subflow-ui-control-buttons");
        var previewButton = $('<button/>', {
            class: "red-ui-button red-ui-button-small"
        }).button().text(RED._("editor.previewUI")).appendTo(buttonDiv);
        var okButton = $('<button/>', {
            class: "red-ui-button red-ui-button-small"
        }).button().text(RED._("editor.previewOK")).appendTo(buttonDiv);

        previewButton.on("click", function (e) {
            e.preventDefault();
            var inputCon = $("#subflow-input-ui");
            var list = envCon.editableList("items");
            var envs = exportEnvList(list, true);
            buildEnvUI(inputCon, null, envs);
            okButton.show();
            previewButton.hide();
            $("#subflow-input-edit-ui").hide();
            localesDiv.hide();
            $("#subflow-dialog-user-count").hide();
            inputCon.show();
        });
        okButton.on("click", function (e) {
            ...
            okButton.hide();
            previewButton.show();
            $("#subflow-input-edit-ui").show();
            localesDiv.show();
            $("#subflow-dialog-user-count").show();
            inputCon.hide();
        });
        okButton.hide();
        $("#subflow-input-env-locale").change();
    }
    // buildEnvControl

    /**
     * Check if template has UI.  Returns definition if exists
     * @param node - subflow node
     * @returns {array} env var definition
     */
    function templateHasUI(node) {
        var subflow = RED.nodes.subflow(node.type.substring(8));
        if (subflow) {
            var envs = subflow.env;
            if (envs) {
                for(var i = 0; i < envs.length; i++) {
                    var env = envs[i];
                    if (env.ui && env.ui.hasUI) {
                        return envs;
                    }
                }
            }
    ...
    /**
     * Revert change for UI definition of env var
     * @param subflow - subflow node
     */
    function revertUIData(subflow) {
        if (subflow.type === "subflow") {
            var envContainer = $('#node-input-env-container');
            var items = envContainer.editableList("items");
            items.each(function (i, v) {
                var item = $(this);
                var row = item.find(".node-input-ui");
                var ui = row.data("ui_");
                var env = row.data("env");
                row.data("ui_", null);
                row.data("ui", ui);
                if (env) {
                    env.ui = ui;
    ...
    function buildEnvForm(container, node) {
        var isTemplate = (node.type === "subflow");
        var envControl = isTemplate ? $("#subflow-input-control") : null;
        var envContainer = $('#node-input-env-container');

        if(envControl) {
            buildEnvControl(envControl, envContainer);
        }
        envContainer
            .css({
                'min-height':'150px',
            ...
            .editableList({
                addItem: function(container, i, opt) {
                    var ui = ((isTemplate && !opt.parent) ? (opt.ui || {
                        open: false,
                        hasUI: true,
                        icon: "",
                        labels: {},
                    ...
                    var row0 = $('<div/>').appendTo(row);
                    var row1 = null;
                    var nameField = null;
                    var expand_button = null;
                    var expand_icon = null;
                    if (ui) {
                        opt.ui = ui;
                        row1 = $('<div/>', {
                            class: "node-input-ui"
                        ...
                        expand_icon = $("<i/>", {
                            class: "fa fa-angle-right red-ui-editor-subflow-env-row"
                        }).appendTo(expand_button);
                        if (ui.open) {
                            row1.show();
                            expand_icon.addClass("down");
                        }
                        else {
                            row1.hide();
                        }
                    ...
                    if (opt.parent) {
                        $('<div/>', {
                            ...
                            style: "margin-left: 5px; width: calc(40% - 20px)",
                        }).appendTo(row0).text(opt.name);
                    } else {
                        nameField = $('<input/>', {
                        ...
                        }).attr("autocomplete","disable").appendTo(row0).val(opt.name);
                    }
                    var valueField = $('<input/>',{
                        class: "node-input-env-value",
                        type: "text",
                        style: "margin-left: 5px; width: calc(60% - 20px)"
                    }).attr("autocomplete","disable").appendTo(row0)

                    var novalType = {
                        value: "noval",
                        label: RED._("editor.types.no-value"),
                        icon: "fa fa-minus",
                        hasValue: false
                    };
                    valueField.typedInput({default:'str',
                                           types:['str','num','bool','json','bin','env',novalType]
                                          });
                    ...
                    if (ui) {
                        buildUIEdit(row1, opt, nameField, valueField);
                        expand_button.on("click", function (evt) {
                            evt.preventDefault();
                            ui.open = !ui.open;
                            if (ui.open) {
                                row1.slideDown();
        ...
        envList.forEach(function(env) {
            envContainer.editableList('addItem', env);
        })
        var templateEnvs = templateHasUI(node);
        if (!isTemplate && templateEnvs) {
            var inputCon = $("#subflow-input-ui");
            buildEnvUI(inputCon, envContainer, templateEnvs, envList);
            $("#subflow-edit-env").hide();
        }
    }
    ...
    function exportEnvList(list, all) {
        if (list) {
            ...
            list.each(function(i) {
                ...
                var ui_con = entry.find(".node-input-ui");
                var ui = ui_con ? ui_con.data("ui") : null;
                if (ui) {
                    var menu_items = entry.find(".ui-menu-item-list");
                    var menu = [];
                    if (menu_items) {
                        var items = menu_items.editableList("items");
                        items.each(function (i, item) {
        ...
        Object.keys(iconSets).forEach(function(moduleName) {
            ...
            if (icons.length > 0) {
                ...
                icons.forEach(function(icon) {
                    ...
                    var colour = node ? RED.utils.getNodeColor(node.type, node._def) : "#3FADB5";
     ...
     * @param default_label - fallback label if not found
     * @param locale - target locale
     * @returns {string} text for specified locale
     */
    function lookupLabel(labels, default_label, locale) {
        if (labels) {
            ...
            if (labels.hasOwnProperty("en-US")) {
                return labels["en-US"];
            }
        }
     ...
     */
    function getEditorLocale() {
        var lc = localStorage.getItem("editor-language");
        if (lc) {
            return lc;
        }
    ...
    function buildSubflowUIRow(opts) {
        var row = $('<tr>');
        var iconCell = $('<td></td>').appendTo(row);
        if (!opts.locked) {
            ...
            iconButton.on("click", function(evt) {
                ...
                var icon = opts.icon || "";
                var iconPath = (icon ? RED.utils.separateIconPath(icon) : {});
                showIconPicker(row, null, iconPath, true, function (newIcon) {
                    console.log(newIcon);
                    iconButton.empty();
                    var path = newIcon || "";
        ...
        if (opts.icon) {
            var newPath = RED.utils.separateIconPath(opts.icon);
            $('<i class="fa '+newPath.file+'"></i>').appendTo(opts.locked?iconCell:iconButton);
        }


        var labelCell = $('<td></td>').appendTo(row);
        var labelSpan = $('<span>').text(opts.label);
        labelSpan.appendTo(labelCell);

        var inputTypesMap = {'str':false,'num':false,'bool':false,'json':false,'bin':false,'env':false};
        if (!opts.types) {
            opts.types = ['str','num','bool','json','bin','env'];
        }
        opts.types.forEach(function(t) { inputTypesMap[t] = true; });

        var inputCell = $('<td></td>').appendTo(row);
        var inputCellInput = $('<div>').appendTo(inputCell);

        var buildInputCell = function() {
            inputCellInput.empty();
            switch(opts.type) {
                case "input":
                    var input = $('<div class="uneditable-input"></div>').appendTo(inputCellInput);
                    if (!opts.locked) {
                        var typeButton = $('<button type="button" class="red-ui-typedInput-type-select"><i class="red-ui-typedInput-icon fa fa-caret-down"></i> </button').appendTo(input).on("click", function(evt) {
                            evt.preventDefault();
                            var menu = RED.menu.init({id:"red-ui-editor-subflow-ui-entry-input-type-menu",
                                options: [
                                    {label: "Select valid types:"},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-str", label: RED._("editor.types.str"), icon: "red/images/typedInput/az.svg",onselect: changeInputType,local:true, selected: inputTypesMap["str"]},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-num", label: RED._("editor.types.num"), icon: "red/images/typedInput/09.svg",onselect: changeInputType,local:true, selected:  inputTypesMap["num"]},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-bool", label: RED._("editor.types.bool"), icon: "red/images/typedInput/bool.svg",onselect: changeInputType,local:true, selected:  inputTypesMap["bool"]},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-json", label: RED._("editor.types.json"), icon: "red/images/typedInput/json.svg",onselect: changeInputType,local:true, selected:  inputTypesMap["json"]},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-bin", label: RED._("editor.types.bin"), icon: "red/images/typedInput/bin.svg",onselect: changeInputType,local:true, selected:  inputTypesMap["bin"]},
                                    {toggle:true, id:"red-ui-editor-subflow-ui-entry-option-env", label: RED._("editor.types.env"), icon: "red/images/typedInput/env.svg",onselect: changeInputType,local:true, selected:  inputTypesMap["env"]}
                                ]
                            });
                            menu.css({
                                position: "absolute"
                            })
                            menu.on('mouseleave', function(){ $(this).remove() });
                            menu.appendTo("body");
                            var elementPos = typeButton.offset();
                            menu.css({
                                top: elementPos.top+"px",
                                left: (elementPos.left - 20)+"px"
                            })
                            menu.show();
                        });
                    }
                    var typeImages = $('<div>').appendTo(input);
                    $('<img src="red/images/typedInput/az.svg" type="str">').toggle(inputTypesMap['str']).appendTo(typeImages);
                    $('<img src="red/images/typedInput/09.svg" type="num">').toggle(inputTypesMap['num']).appendTo(typeImages);
                    $('<img src="red/images/typedInput/bool.svg" type="bool">').toggle(inputTypesMap['bool']).appendTo(typeImages);
                    $('<img src="red/images/typedInput/json.svg" type="json">').toggle(inputTypesMap['json']).appendTo(typeImages);
                    $('<img src="red/images/typedInput/bin.svg" type="bin">').toggle(inputTypesMap['bin']).appendTo(typeImages);
                    $('<img src="red/images/typedInput/env.svg" type="env">').toggle(inputTypesMap['env']).appendTo(typeImages);
                    break;
                case "select":
                    var select = $('<div class="uneditable-input"></div>').appendTo(inputCellInput);
                    if (!opts.locked) {
                        var selectOptionButton = $('<button type="button" class="red-ui-typedInput-option-trigger"><i class="red-ui-typedInput-icon fa fa-caret-down"></i> </button').appendTo(select).on("click", function(evt) {
                            evt.preventDefault();
                        });
                    }
                    ...
                    break;
                case "checkbox":
                    $('<input type="checkbox">').appendTo(inputCellInput);
                    labelSpan.appendTo(inputCellInput);
                    break;
                case "spinner":
                    $('<input type="input" disabled>').appendTo(inputCellInput).spinner({disabled:true}).parent().removeClass("ui-state-disabled");
                    break;
                case "hidden":
                break;
            }
        ...
        if (!opts.locked) {
            var changeInputType = function(selected) {
                var type = this.id.substring(38);
                inputTypesMap[type] = selected;
                inputCellInput.find('[type="'+type+'"]').toggle(selected)
            }
            var changeType = function(selected) {
                if (selected && opts.type !== this.type) {
                    opts.type = this.type;
                    buildInputCell();
                } else if (!selected && this.type === "checkbox" && opts.type === "checkbox") {
                    // deselected checkbox
                    labelSpan.appendTo(labelCell);
                }
            }
            var optButton = $('<button type="button" class="options-button red-ui-button red-ui-button-small"><i class="fa fa-caret-down"></i></button>').appendTo(inputCell).on("click", function(evt) {
                evt.preventDefault();
                var menu = RED.menu.init({id:"red-ui-editor-subflow-ui-entry-option-menu",
                    options: [
                        {label: "Select input type:"},
                        {toggle:"red-ui-editor-subflow-ui-entry-option",id:"red-ui-editor-subflow-ui-entry-option-input",label:"input",type:"input",selected:opts.type==="input",onselect: changeType,local:true},
                        {toggle:"red-ui-editor-subflow-ui-entry-option",id:"red-ui-editor-subflow-ui-entry-option-select",label:"select",type:"select",selected:opts.type==="select",onselect: changeType,local:true},
                        {toggle:"red-ui-editor-subflow-ui-entry-option",id:"red-ui-editor-subflow-ui-entry-option-checkbox",label:"checkbox",type:"checkbox",selected:opts.type==="checkbox",onselect: changeType,local:true},
                        {toggle:"red-ui-editor-subflow-ui-entry-option",id:"red-ui-editor-subflow-ui-entry-option-spinner",label:"spinner",type:"spinner",selected:opts.type==="spinner",onselect: changeType,local:true},
                        {toggle:"red-ui-editor-subflow-ui-entry-option",id:"red-ui-editor-subflow-ui-entry-option-hidden",label:"hidden",type:"hidden",selected:opts.type==="hidden",onselect: changeType,local:true}
                    ]
                });
                menu.css({
                    position: "absolute"
                ...
                menu.on('mouseleave', function(){ $(this).remove() });
                menu.on('mouseup', function() { setTimeout(function() { menu.remove() },10) });
                menu.appendTo("body");
                var elementPos = optButton.offset();
                menu.css({
                    top: elementPos.top+"px",
                    left: (elementPos.left - menu.width() + 20)+"px"
                })
                menu.show();
            });
        }
        ...
        var controlCell = $('<td></td>').appendTo(row);
        if (!opts.locked) {
            $('<button type="button" class="red-ui-button red-ui-button-small"><i class="fa fa-times"></i></button>').appendTo(controlCell);
        }
        return row;

    }
    function buildSubflowUIForm(container,node) {
        var dialogForm = $('<form class="dialog-form form-horizontal" autocomplete="off"></form>').appendTo(container);

        var gridContainer = $('<table class="red-ui-editor-subflow-ui-grid"></table>').appendTo(dialogForm);

        var row = buildSubflowUIRow({
        ...
        // <div class="form-row"><label for="node-input-name" data-i18n="[append]editor:common.label.name"><i class="fa fa-tag"></i> Name</label><input type="text" id="node-input-name" autocomplete="disable" dir=""></div>
    ...
    function showEditDialog(node) {
        ...
        var trayOptions = {
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (type === "subflow") {
                            ...
                            var list = $("#node-input-env-container").editableList("items");
                            var new_env = exportEnvList(list);
            ...
            close: function() {
                if (restoreLocaleTip) {
                    restoreLocaleTip.close()
                    restoreLocaleTip = null;
                }
    ...
    function showEditSubflowDialog(subflow) {
        ...
        var trayOptions = {
            ...
            resize: function(size) {
                ...
                $("#node-input-env-container").editableList('height',height-70);
            },
            open: function(tray) {
                var trayFooter = tray.find(".red-ui-tray-footer");
                ...
                var uiTab = {
                    id: "editor-tab-ui",
                    label: "UI",//RED._("editor-tab.appearance"),
                    name: "UI",//RED._("editor-tab.appearance"),
                    content: $('<div>', {class:"red-ui-tray-content"}).appendTo(editorContent).hide(),
                ...
                buildSubflowUIForm(uiTab.content,editing_node);
                editorTabs.addTab(uiTab);
