...
RED.editor = (function() {
    ...
    function initEditor(){
        //console.log("initEditor->initEditor() called");
        try {
            var editorSettings = getEditorSettings();
            //console.log("initEditor->editorSettings: ", editorSettings);
    ...
    function registerJsonata(_monaco) {
        ...
        _monaco.languages.registerHoverProvider('jsonata', {
            provideHover: function (model, position) {
                console.log("hover provider position - ", position);
                ...
                console.log("hover provider sees - " + f);
                var args = RED._('jsonata:' + f + ".args", { defaultValue: '' });
                if (!args) return;
                var title = f + "(" + args + ")";
                var body = RED._('jsonata:' + f + '.desc', { defaultValue: '' });
    ...
    function initMonaco(){
        function monacoSnippet(label, insertText, documentation, range) {
        ...
        window.MonacoEnvironment = {
            getWorkerUrl: function(moduleId, label) {
                //console.log(`window.MonacoEnvironment.getWorkerUrl ${label}`)
                if (label === 'json') {
        ...
        //console.log("initMonaco->editorSettings:", editorSettings);
        ...
        function createJSSnippets(range) {
            return [        
                monacoSnippet("for (for loop)", 'for (let ${1:index} = 0; ${1:index} < ${2:array}.length; ${1:index}++) {\n\tconst element = ${2:array}[${1:index}];\n\t${3}\n}','for loop',range),
                monacoSnippet("foreach", '${1:array}.forEach(function(${2:element}) {\n\t${3}\n});','forEach(callbackfn: (value: T, index: number, array: readonly T[]) => void, thisArg?: any): void\n\nA function that accepts up to three arguments. forEach calls the callbackfn function one time for each element in the array.',range),
                monacoSnippet("forin", 'for (${1:prop} in ${2:obj}) {\n\tif (${2:obj}.hasOwnProperty(${1:prop})) {\n\t\t${3}\n\t}\n}','for in',range),
                monacoSnippet("forof", 'for (const ${1:iterator} of ${2:object}) {\n\t${3}\n}','for of',range),
                monacoSnippet("function", 'function ${1:methodName}(${2:arguments}) {\n\t${3}\n}','Function Declaration',range),
                monacoSnippet("func (anonymous function)", 'var ${1:fn} = function(${2:arguments}) {\n\t${3}\n}','Function Expression',range),
                monacoSnippet("pt (prototype)", '${1:ClassName}.prototype.${2:methodName} = function(${3:arguments}) {\n\t${4}\n}','prototype',range),
                monacoSnippet("iife", '(function(${1:arg}) {\n\t${1}\n})(${1:arg});','immediately-invoked function expression',range),
                monacoSnippet("call (function call)", '${1:methodName}.call(${2:context}, ${3:arguments})','function call',range),
                monacoSnippet("apply (function apply)", '${1:methodName}.apply(${2:context}, [${3:arguments}])','function apply',range),
                monacoSnippet("jsonparse", 'JSON.parse(${1:obj});','JSON.parse',range),
                monacoSnippet("jsonstringify", 'JSON.stringify(${1:obj});','JSON.stringify',range),
                monacoSnippet("setinterval", 'setInterval(function() {\n\t${2}\n}, ${1:delay});','setInterval',range),
                monacoSnippet("settimeout", 'setTimeout(function() {\n\t${2}\n}, ${1:delay});','setTimeout',range),
                monacoSnippet("node.warn", 'node.warn(${1:"my warning"});','send a warning to the debug side bar',range),
                monacoSnippet("node.error", 'node.error(${1:err}, ${2:msg});','send an error to the debug side bar',range),
                monacoSnippet("node.send", 'node.send(${1:msg});','async send a msg to the next node',range),
                monacoSnippet("node.send (multiple)", 'var ${1:msg1} = {payload:${2:1}};\nvar ${3:msg2} = {payload:${4:2}};\nnode.send([[${1:msg1}, ${3:msg2}]]);','send 1 or more messages out of 1 output',range),
                monacoSnippet("node.send (multiple outputs)", 'var ${1:msg1} = {payload:${2:1}};\nvar ${3:msg2} = {payload:${4:2}};\nnode.send([${1:msg1}, ${3:msg2}]);','send more than 1 message out of multiple outputs',range),
                monacoSnippet("node.status", 'node.status({fill:"${1:red}",shape:"${2:ring}",text:"${3:error}"});','Set the status icon and text underneath the function node',range),
                monacoSnippet("cloneMessage (RED.util)", 'RED.util.cloneMessage(${1:msg});',
                    ["```typescript",
                    ...
                    range),
                monacoSnippet("getObjectProperty (RED.util)", 'RED.util.getObjectProperty(${1:msg},${2:prop});',
                    ["```typescript",
                    ...
                    range),
                monacoSnippet("setObjectProperty (RED.util)", 'RED.util.setObjectProperty(${1:msg},${2:prop},${3:value},${4:createMissing});',
                    ["```typescript",
                    ...
                    range),
                monacoSnippet("getMessageProperty (RED.util)", 'RED.util.getMessageProperty(${1:msg},${2:prop});',
                    ["```typescript",
                    ...
                    "*@param* `prop` — the msg object"],
                    range),
                monacoSnippet("setMessageProperty (RED.util)", 'RED.util.setMessageProperty(${1:msg},${2:prop},${3:value},${4:createMissing});',
                    ["```typescript",
        ...
        //TODO: figure out how to use RED.util types for better itellisense
        /* Useful info...
        https://stackoverflow.com/questions/52290727/adding-typescript-type-declarations-to-monaco-editor
        https://github.com/search?l=JavaScript&p=2&q=addExtraLib&type=Code
        https://github.com/blnqco/blnq-studio/blob/5466dda049a4fd1d673a52654fc46d4f96cb380e/services/types/index.js

        this looks promising. Would require "require"
        https://www.gitmemory.com/issue/microsoft/monaco-editor/1622/539435699

        Working example (paste the below into https://microsoft.github.io/monaco-editor/playground.html#extending-language-services-configure-javascript-defaults)...
        ...
        var nodeGlobalsURL = "https://raw.githubusercontent.com/DefinitelyTyped/DefinitelyTyped/master/types/node/globals.d.ts"

        makeRequest('GET', nodeGlobalsURL).then(function(data){
            //debugger
            const nodeTS = data;
            const nodeGlobalsUri = '@types/node/globals.d.ts';
            ...
            monaco.languages.typescript.typescriptDefaults.addExtraLib(nodeTS, nodeGlobalsUri);
            monaco.languages.typescript.javascriptDefaults.addExtraLib(nodeTS, nodeGlobalsUri);

            var jsCode = 'var b = Buffer.from([1,2,3,4,5]);';
        ...
        */
        ...
        registerJsonata(monaco);


        //TODO: split out to file. also add Buffer and other objects (better still use @types)
        var nodeLib = ' \
        ...
        \n\ndeclare class node { \
            ...
            \n* @example ` \
            \n* ```javascript  \
            \n* //set node status to red ring with text \
            \n* node.status({fill:"red",shape:"ring",text:"disconnected"});\
            \n* ``` \
            \n* @see {@link https://nodered.org/docs/user-guide/writing-functions | docs} for more info \
            \n*/ \
        ...
        try {
            ...
            console.warn("monaco - Error setting javascriptDefaults", err)
        }
        ...
        try {
            ...
            console.warn("monaco - Error setting CompilerOptions", err)
        }
     }
    ...
    function createMonacoEditor(options) {
        var editorSettings = getEditorSettings()
        //console.log("createMonacoEditor -> editorSettings = ", editorSettings);
        
        var createThemeMenuOption = function(theme){
            return {
        ...
        var getMonacoLanguage = function(mode) {
            ...
            mode = mode.replace("nrjavascript","javascript");
            return mode;
        }
        //console.log("createEditor(options) options=>", options);
        ...
        function setMode(mode, cb) {
            ...
            //console.log("setMode(mode, cb) ",mode,options);
        ...
        function gotoLine(row,col){
            //console.log("gotoLine(row,col) " + row + "," + col);
        ...
        ed.on = function(name, cb){
            ...
            //console.log("monaco - wiring up event " + name + " to ", cb);
            fn(cb);
        }
        ...
        //console.log("returning editor instance for " + editorOptions.language)
        ed.isMONACO = true;
        return ed;
