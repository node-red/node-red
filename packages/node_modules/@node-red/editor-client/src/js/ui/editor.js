...
RED.editor = (function() {
    ...
    function buildInputUI(container, params, map, locale) {
        container.empty();
        if (!params) {
            return;
        }
        params.forEach(function (item) {
            ...
            var entry = (map && map.hasOwnProperty(name)) ? map[name] : null; 
            var type = item.type;
            var def_type = entry ? entry.type : undefined;
            var value = entry ? entry.value : item.value;
            var row0 = $("<div/>", {
                class: "form-row",
            ...
            if ((type === "str") || (type === "num") ||
                ...
                valueField.on("change", function (e, t) {
                    ...
                    map[name] = { type: type, value: new_val };
                });
                valueField.change();
            }
                ...
                valueField.on("change", function (e, t) {
                    ...
                    map[name] = { type: new_type, value: new_val };
                });
                valueField.change();
            }
                ...
                items.forEach(function (item) {
                    var value = lookupLabel(item.values, item.value, locale);
                    $("<option>", {
                        value: item.value
    ...
    function exportUISetting(list) {
        ...
        list.each(function (i, item) {
            var entry = $(this);
            var name = entry.find(".params-name").val();
    ...
    function loadParams(container, params) {
        if (params) {
    ...
    function buildUiEditForm(container, input_con, node) {
        var ui_in_tmp = copyMap(node.ui_in);
        ...
        //         <div id="ui-edit-buttons" class="editor-tray-toolbar"/>
        ...
        var edit_buttons = $("<div/>", {
            "id": "ui-edit-buttons",
            "class": "editor-tray-toolbar"
        }).appendTo(ui_edit);
        var button_apply = null;

        var localesList  = RED.settings.subflowLocales;
        $("<span/>", {
            style: "float: left; padding-top: 7px;"
        }).text(RED._("editor-tab.locale")).appendTo(edit_buttons);
        var locales = $("<select/>", {
            style: "width: 140px; float: left; margin-left: 8px;"
        }).appendTo(edit_buttons);
        localesList.forEach(function(item) {
            $("<option/>", {
                value: item.locale
            }).text(item.name).appendTo(locales);
        });
        if (input_con) {
            button_apply  = $('<button/>').button().appendTo(edit_buttons);
        }
        ...
        //         <div id="ui-preview-buttons" class="editor-tray-toolbar"/>
        ...
        if (input_con) {
            button_apply.text(RED._("editor-tab.apply"));
            button_apply.click(function(e) {
                e.preventDefault();
                var ui_def_list = exportUISetting(edit_con.editableList("items"));
                buildInputUI(input_con, ui_def_list, ui_in_tmp, editorLocale);
            });
        }
        ...
        button_preview.click(function(e) {
            e.preventDefault();
            var ui_def_list = exportUISetting(edit_con.editableList("items"));
            buildInputUI(preview_con, ui_def_list, {}, currentLocale);
            ui_edit.hide();
            ui_preview.show();
        });
    ...
    return {
        init: function() {
            ...
            editorLocale = i18n.detectLanguage();
            editorLang = editorLocale ? editorLocale.substring(0, 2) : null;
        },
        edit: showEditDialog,
