...
RED.editor = (function() {
     ...
     */
     function buildEnvEditRow(container, env, nameField, valueField) {
         ...
         if (env.ui.opts) {
             typeOptions[env.ui.type] = env.ui.opts;
         } else {
             // Pick up the default values if not otherwise provided
             env.ui.opts = typeOptions[env.ui.type];
         }
         ...
         if (env.ui.icon) {
             var newPath = RED.utils.separateIconPath(env.ui.icon);
             $('<i class="fa '+newPath.file+'"></i>').appendTo(iconButton);
         }

         var labelCell = $('<div></div>').appendTo(row);

         var label = env.ui.label && env.ui.label[currentLocale] || "";
         var labelInput = $('<input type="text">').val(label).appendTo(labelCell);
         env.ui.labelField = labelInput;
         labelInput.on('change', function(evt) {
             env.ui.label = env.ui.label || {};
             var val = $(this).val().trim();
             if (val === "") {
                 delete env.ui.label[currentLocale];
             } else {
                 env.ui.label[currentLocale] = val;
             }
         })
         ...
         RED.popover.tooltip(labelIcon,function() {
             var langs = Object.keys(env.ui.label);
             var content = $("<div>");
             ...
             langs.forEach(function(l) {
                 ...
                 $('<span>').text(env.ui.label[l]||"").appendTo(row);
             });
             return content;
         })
         ...
         inputCellInput.typedInput({
                 ...
                 {value:"input",
                    label:"input", icon:"fa fa-i-cursor",showLabel:false,multiple:true,options:[
                     ...
                     valueLabel: function(container,value) {
                         ...
                         $('<i class="fa fa-i-cursor"></i>').appendTo(input);
                         if (value.length) {
                             value.forEach(function(v) {
                                 $('<img>',{src:v.icon,style:"max-width:14px; padding: 0 3px; margin-top:-4px; margin-left: 5px"}).appendTo(input);
                             })
                         } else {
                             $("<span>").css({
                                 "color":"#aaa",
                 ...
                 {value:"spinner",
                    label:"spinner", icon:"fa fa-sort-numeric-asc", showLabel:false,
                     valueLabel: function(container,value) {
                         ...
                         var input = $('<div class="placeholder-input">').appendTo(innerContainer);
                         $('<i class="fa fa-sort-numeric-asc"></i>').appendTo(input);

                         var min = env.ui.opts && env.ui.opts.min;
                         var max = env.ui.opts && env.ui.opts.max;
                         var label = "";
                     ...
                     expand: {
                         ...
                         content: function(container) {
                             ...
                             var min = env.ui.opts.min;
                             var max = env.ui.opts.max;
                             ...
                             return {
                                 onclose: function() {
                                     ...
                                     if (min !== "") {
                                         env.ui.opts.min = parseInt(min);
                                     } else {
                                         delete env.ui.opts.min;
                                     }
                                     if (max !== "") {
                                         env.ui.opts.max = parseInt(max);
                                     } else {
                                         delete env.ui.opts.max;
                                     }
                                     inputCellInput.typedInput('value',Date.now())
                                 }
                             }
             ...
             env.ui.type = $(this).typedInput("type");
             env.ui.opts = typeOptions[env.ui.type];
             if (env.ui.type === 'input') {
                 ...
                 // selected options.
                 inputCellInput.typedInput('value',env.ui.opts.types.join(","))
             } else {
                 // No other type cares about `value`, but doing this will
                 // force a refresh of the label now that `env.ui.opts` has
                 // been updated.
                 inputCellInput.typedInput('value',Date.now())
             }
    ...
    function buildEnvUI(uiContainer, envList) {
        ...
        for (var i = 0; i < envList.length; i++) {
            ...
            var labelContainer = $('<span></span>').css({"display":"inline-block","width":"16px"}).appendTo(label);
            if (ui.icon) {
    ...
    function buildEnvForm(container, node) {
        ...
        envContainer
            ...
            .editableList({
                header: $('<div><div><div></div><div>Name</div><div>Default value</div><div></div></div></div>'),
    ...
    function exportSubflowInstanceEnv(node) {
        var fullEnv = getSubflowInstanceEnv(node);
        fullEnv.forEach(function(env) {
            var ui = env.ui || {};
            if (!ui.type) {
                ui.type = "input";
                ui.opts = {types:['str','num','bool','json','bin','env']}
            ...
            var input = $("#"+getSubflowEnvPropertyName(env.name));
            if (input.length) {
                switch(ui.type) {
                    case "input":
                        if (ui.opts.types && ui.opts.types.length > 0) {
                            env.value = input.typedInput('value');
                            env.type = input.typedInput('type');
                        } else {
                            env.value = input.val();
                            env.type = 'str';
                        }
    ...
    function buildEditForm(container,formId,type,ns,node) {
        ...
        if (type === "subflow-template") {
            ...
            buildEnvUI($("#subflow-input-ui"), getSubflowInstanceEnv(node));
        }
    ...
    function getSubflowInstanceEnv(node) {
        var parentEnv = {};
        var envList = [];
        if (/^subflow:/.test(node.type)) {
            var subflowDef = RED.nodes.subflow(node.type.substring(8));
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (type === "subflow") {
                            ...
                            var new_env = exportSubflowInstanceEnv(editing_node);
