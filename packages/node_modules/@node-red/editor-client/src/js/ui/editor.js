 ...
 */
RED.editor = (function() {
    //INFO: USE_MONACO: A flag that is set after settings are avialable - to determine if an ACE edit
    //INFO: monacoThemes: The themes to make avaiable in F1 menu
    var USE_MONACO;
    ...
    function getEditorSettings() {
        ...
        //console.log("getEditorSettings - RED.settings.editor is empty. Defaulting to MONACO"); //TODO: remove
        return {
            lib: "MONACO",
            theme: "vs"
        };
    ...
    function initEditor() {
        //console.log("initEditor->initEditor() called"); //TODO: remove
        try {
            var editorSettings = getEditorSettings();
            //console.log("initEditor->editorSettings: ", editorSettings); //TODO: remove
            USE_MONACO = editorSettings.lib === "MONACO" || false;
            if (USE_MONACO) {
                initMonaco();
            }
        } catch (error) {
    ...
    function initMonaco() {
        ...
        window.MonacoEnvironment = {
            getWorkerUrl: function (moduleId, label) {
                //console.log(`window.MonacoEnvironment.getWorkerUrl ${label}`) //TODO: remove
                if (label === 'json') {
                    return './vendor/monaco/dist/json.worker.js';
                }
                if (label === 'css' || label === 'scss') {
                    return './vendor/monaco/dist/css.worker.js';
                }
                if (label === 'html' || label === 'handlebars') {
                    return './vendor/monaco/dist/html.worker.js';
                }
                if (label === 'typescript' || label === 'javascript') {
                    return './vendor/monaco/dist/ts.worker.js';
                }
        ...
        //console.log("initMonaco->editorSettings:", editorSettings); //TODO: remove
            ...
            if (!monacoThemes.includes(editorOptions.theme)) {
                ...
                $.get(customTheme, function (theme) {
                    ...
                    if ((theme.rules && Array.isArray(theme.rules)) || theme.colors) {
                        ...
                        //console.log("initMonaco->custom monaco theme doent contain 'rules' or 'colors'") //TODO: remove
                    }
                });
            } else {
                //console.log("initMonaco->No need to load custom theme: ", editorOptions.theme );  //TODO: remove
            }
        }

        //Helper function to simplify snippet setup
        function createMonacoSnippet(label, insertText, documentation, range) {
        ...
        function createJSSnippets(range) {
            return [        
                createMonacoSnippet("for (for loop)", 'for (let ${1:index} = 0; ${1:index} < ${2:array}.length; ${1:index}++) {\n\tconst element = ${2:array}[${1:index}];\n\t${3}\n}','for loop',range),
                createMonacoSnippet("foreach", '${1:array}.forEach(function(${2:element}) {\n\t${3}\n});','forEach(callbackfn: (value: T, index: number, array: readonly T[]) => void, thisArg?: any): void\n\nA function that accepts up to three arguments. forEach calls the callbackfn function one time for each element in the array.',range),
                createMonacoSnippet("forin", 'for (${1:prop} in ${2:obj}) {\n\tif (${2:obj}.hasOwnProperty(${1:prop})) {\n\t\t${3}\n\t}\n}','for in',range),
                createMonacoSnippet("forof", 'for (const ${1:iterator} of ${2:object}) {\n\t${3}\n}','for of',range),
                createMonacoSnippet("function", 'function ${1:methodName}(${2:arguments}) {\n\t${3}\n}','Function Declaration',range),
                createMonacoSnippet("func (anonymous function)", 'var ${1:fn} = function(${2:arguments}) {\n\t${3}\n}','Function Expression',range),
                createMonacoSnippet("pt (prototype)", '${1:ClassName}.prototype.${2:methodName} = function(${3:arguments}) {\n\t${4}\n}','prototype',range),
                createMonacoSnippet("iife", '(function(${1:arg}) {\n\t${1}\n})(${1:arg});','immediately-invoked function expression',range),
                createMonacoSnippet("call (function call)", '${1:methodName}.call(${2:context}, ${3:arguments})','function call',range),
                createMonacoSnippet("apply (function apply)", '${1:methodName}.apply(${2:context}, [${3:arguments}])','function apply',range),
                createMonacoSnippet("jsonparse", 'JSON.parse(${1:obj});','JSON.parse',range),
                createMonacoSnippet("jsonstringify", 'JSON.stringify(${1:obj});','JSON.stringify',range),
                createMonacoSnippet("setinterval", 'setInterval(function() {\n\t${2}\n}, ${1:delay});','setInterval',range),
                createMonacoSnippet("settimeout", 'setTimeout(function() {\n\t${2}\n}, ${1:delay});','setTimeout',range),
                createMonacoSnippet("node.warn", 'node.warn(${1:"my warning"});','send a warning to the debug side bar',range),
                createMonacoSnippet("node.error", 'node.error(${1:err}, ${2:msg});','send an error to the debug side bar',range),
                createMonacoSnippet("node.send", 'node.send(${1:msg});','async send a msg to the next node',range),
                createMonacoSnippet("node.send (multiple)", 'var ${1:msg1} = {payload:${2:1}};\nvar ${3:msg2} = {payload:${4:2}};\nnode.send([[${1:msg1}, ${3:msg2}]]);','send 1 or more messages out of 1 output',range),
                createMonacoSnippet("node.send (multiple outputs)", 'var ${1:msg1} = {payload:${2:1}};\nvar ${3:msg2} = {payload:${4:2}};\nnode.send([${1:msg1}, ${3:msg2}]);','send more than 1 message out of multiple outputs',range),
                createMonacoSnippet("node.status", 'node.status({fill:"${1:red}",shape:"${2:ring}",text:"${3:error}"});','Set the status icon and text underneath the function node',range),
                createMonacoSnippet("cloneMessage (RED.util)", 'RED.util.cloneMessage(${1:msg});',
                    ["```typescript",
                    ...
                    range),
                createMonacoSnippet("getObjectProperty (RED.util)", 'RED.util.getObjectProperty(${1:msg},${2:prop});',
                    ["```typescript",
                    ...
                    range),
                createMonacoSnippet("setObjectProperty (RED.util)", 'RED.util.setObjectProperty(${1:msg},${2:prop},${3:value},${4:createMissing});',
                    ["```typescript",
                    ...
                    range),
                createMonacoSnippet("getMessageProperty (RED.util)", 'RED.util.getMessageProperty(${1:msg},${2:prop});',
                    ["```typescript",
                    ...
                    "*@param* `prop` — the msg object"],
                    range),
                createMonacoSnippet("setMessageProperty (RED.util)", 'RED.util.setMessageProperty(${1:msg},${2:prop},${3:value},${4:createMissing});',
                    ["```typescript",
        ...
        function setupJsonata(_monaco) {
            ...
            _monaco.languages.registerCompletionItemProvider('jsonata', {
                ...
                provideCompletionItems: function (model, position) {
                    ...
                    //console.log(word, range) //TODO: Remove
        ...
        function setupJSON(_monaco) {
            ...
            try {
                _monaco.languages.json.jsonDefaults.setDiagnosticsOptions({validate: true});
            } catch (error) {
                console.warn("monaco - Error setting json validate true", err)
            }  
        }
        ...
        function setupJS(_monaco) {
            ...
            try {                        
                ...
                var compilerOptions = {
                    target: _monaco.languages.typescript.ScriptTarget.ES6,
        ...
        monaco.__nodered_extraLib_URL = "./vendor/monaco/dist/node-red-extra.d.ts";
        monaco.__nodered_extraLib_Uri = '@types/node-red/node-red-extra.util.d.ts';
        ...
        $.get(monaco.__nodered_extraLib_URL, function (data) {
            //console.log("initMonaco --> setupJS --> $.get --> data"); //TODO: Remove
            monaco.__extra_serversideTS = data;
        });
    ...
    function createEditor(options) {
        ...
        if (USE_MONACO) {
            ...
            if (!options.element && !options.id) options.id = 'node-backwards-compatability-dummy-editor';
            ...
            return createACEEditor(options);
        }
    }

    function createMonacoEditor(options) {
        var editorSettings = getEditorSettings()
        //console.log("createMonacoEditor -> editorSettings = ", editorSettings); //TODO: remove
        ...
        //console.log(ua,browser,os,touch,mobile,tablet); //TODO: Remove
        if(mobile || tablet) {
            editorOptions.minimap = { enabled: false };
            editorOptions.formatOnType = false;
            editorOptions.formatOnPaste = false;
            editorOptions.disableMonospaceOptimizations = true;
            editorOptions.codeLens = false;
            editorOptions.columnSelection = false;
            editorOptions.matchBrackets = "never";
            editorOptions.maxTokenizationLineLength = 10000;
            editorOptions.roundedSelection = false;
            editorOptions.quickSuggestions = true;
            editorOptions.stopRenderingLineAfter = 4000;
            editorOptions.trimAutoWhitespace = false;
            editorOptions.parameterHints = {cycle:true, enabled:false};
            editorOptions.value = "function hello() {\n\talert('Hello world!');\n}";
        ...
        //if clientSideSuggestions option is not specified, check see if requested mode is "nrjavascript"
        var serverSideSuggestions = false;
        if (options.clientSideSuggestions == null) {
            serverSideSuggestions = (options.mode + "").indexOf("nrjavascript") >= 0;
        }
        ...
        if (serverSideSuggestions) {
            ...
            //(re)load (if necessary) the serverside "addExtraLib"s 
        ...
        ed.setMode = setMode.bind(ed);

        //#region "ACE compatability"

        ed.getRange = getRange.bind(ed);
        ...
        function getRange(){
            ...
            */
            r.start = {
                row: r.selectionStartLineNumber,
        ...
        ed.selection = {};
        ed.selection.getRange = getRange.bind(ed);

        ed.session = {};
        ed.session.insert = editer_insert.bind(ed);
        function editer_insert(position, text) {
            var range = new monaco.Range(position.row, position.column, position.row, position.column);
            var id = { major: 1, minor: 1 };
            var op = { identifier: id, range: range, text: text, forceMoveMarkers: true };
            this.executeEdits("session-insert", [op]);
        }

        ed.session.replace = editer_replace.bind(ed);
        function editer_replace(range, text) {
            var id = { major: 1, minor: 1 };
            var op = { identifier: id, range: range, text: text, forceMoveMarkers: true };
            this.executeEdits("session-insert", [op]);
        }
        ...
        function getSelectedText() {
            return this.getModel().getValueInRange(this.getSelection());
        }
        ed.getSelectedText = getSelectedText.bind(ed);

        ed.insertSnippet = editer_insertSnippet.bind(ed);
        function editer_insertSnippet(s) {
            //https://github.com/microsoft/monaco-editor/issues/1112#issuecomment-429580604
            //no great way of triggering snippets!
            let contribution = this.getContribution("snippetController2");
        ...
        if (!ed.destroy) {
            ed.destroy = destroy.bind(ed);
        }

        function resize() {
            //console.log("resize() " + this._mode); //TODO: remove
            this.layout();
        }
        ed.resize = resize.bind(ed);

        function getSession() {
            return this;
        }
        ed.getSession = getSession.bind(ed);

        function moveCursorTo(lineNumber, colNumber) {
            this.setPosition({ lineNumber: lineNumber, column: colNumber });
        }
        ed.moveCursorTo = moveCursorTo.bind(ed);
        ...
        function getAnnotations() {
            ...
            if (_model !== null) {
                var modeId = _model.getModeId();
                _markers = monaco.editor.getModelMarkers(_model) || [];
                _markers.filter(function (marker) {
                    return marker.owner = modeId
                })

                _markers = _markers.map(function (marker) {
                    return {
                        row: marker.startLineNumber,
                        text: marker.message,
                        type: monaco.MarkerSeverity[marker.severity] ? monaco.MarkerSeverity[marker.severity].toLowerCase() : marker.severity
                    }
                })
            }
            return _markers || [];
        ...
        ed.getAnnotations = getAnnotations.bind(ed);

        function gotoLine(row, col) {
            //console.log("gotoLine(row,col) " + row + "," + col); //TODO: remove
            this.setPosition({ lineNumber: row, column: col });
        }
        ed.gotoLine = gotoLine.bind(ed);

        function getCursorPosition() {
            var p = this.getPosition();
            return { row: p.lineNumber, column: p.column };
        }
        ed.getCursorPosition = getCursorPosition.bind(ed);
        ...
        function getUndoManager() {
            ...
            function isClean() {
                try {
                    return this.getModel().canUndo() === false
                } catch (error) {
                    return false
                }

            }
            o.isClean = isClean.bind(this);
            return o;
        }
        ed.getUndoManager = getUndoManager.bind(ed)
