...
RED.editor = (function() {
    ...
    /**
     * Setup interface for UI editing
     * @param cont - container
     * @param height - minimum height of container
     */
    function setupUIEditContainer(cont, height) {
        // definitions for typedInput fields
        // anyType: selection from types using typedInput interface
        var anyType = {
            value: "any",
            label: "any",
            icon: "fa fa-cog",
        ...
        // spinnerType: numeric input using spinner
        var spinnerType = {
            value: "spinner",
            label: "spinner",
            icon: "fa fa-exchange",
        ...
        // checkboxType: checkbox interface (label is displayed after check)
        var checkboxType = {
            value: "checkbox",
            label: "checkbox",
        ...
        // menuType: selection from menu items
        var menuType = {
            value: "menu",
            label: "menu",
            icon: "fa fa-list",
        ...
        // labelType: only show label (no input)
        var labelType = {
            value: "label",
            label: "label",
            icon: "fa fa-tag",
        ...
        cont
            ...
            .editableList({
                addItem: function (container0, i, opt) {
                    /**
                     * Setup Menu UI definition 
                     * @param row - enclosing row
                    ...
                    /**
                     * Setup Types Selection UI for typedInput based inputs
                     * @param row - enclosing row
                     * @param selected - selected types
                     * @param valeu - default value
                     */
                    function setupTypeSelector(row, selected, value) {
                        var type_map = {
                            "str" : {
                                label: "string",
                        ...
                        var count = 0;
                        var div0 = $("<div/>", {
                            style: "margin-left: 15px; widrh: 360px;"
                        }).appendTo(row);
                        types.forEach(function (type) {
                            ...
                            var check = $("<input/>", {
                                ...
                                value: type,
                                class: "type-selector",
                                id: type +"_" +_ui_elem_count,
                                style: "width: auto; margin-left: 5px; margin-right: 5px;"
                            }).appendTo(div2);
                            if (selected && (selected.indexOf(type) >= 0)) {
                    ...
                    var nameField = $("<input/>", {
                        ...
                        placeholder: RED._("editor-tab.env-var")
                    }).appendTo(row0);
                    if (opt && opt.name) {
                        nameField.val(opt.name);
                    }
                    if (opt && opt.id) {
                        nameField.data("id", opt.id);
                    }
                    ...
                    var labelField = $("<input/>", {
                        ...
                        type: "text",
                        style: "width:30%; margin-left:5px;",
                        placeholder: RED._("editor-tab.label")
                    }).appendTo(row1);
                    if (opt) {
                        var labels = opt.labels ? opt.labels : {};
                        labelField.data("labels", labels);
                    ...
                    var row_menu = $("<div/>", {
                        ...
                        style: "min-height: 100px; margin-top: 10px; margin-left: auto; margin-right: 7px;"
                    }).appendTo(container0);
                    setupMenuItems(row_menu, opt.value, opt.items);

                    // row: any
                    var row_any = $("<div/>").appendTo(container0);
                    setupTypeSelector(row_any, opt.items, opt.value);
                    ...
                    valueField.change(function () {
                        ...
                        row_menu.toggle(type === "menu");
                        row_any.toggle(type === "any");
                    });
                    valueField.change();
                },
                removeItem: function (opt) {
    ...
    } // setupUIEditContainer

    /**
     * Lookup value and type for specified id
     * @param map - mapping from id to entry
     * @item - item to be looked up
     * @returns {object} containing type and value
     */
    function lookupValue(map, item) {
        var id = item.id;
        var entry = item;
    ...
    /**
     * Build UI for inputting values for environment variables  
     * @param container - enclosing container
     * @param ui - UI definition
     * @param map - mapping of values
     * @param locale - target locale
     */
    function buildInputUI(container, ui, map, locale) {
        container.empty();
        if (!ui || !ui.params) {
            return;
        }
        ...
        params.forEach(function (item) {
            var id = item.id;
            var name = item.name;
            var label = lookupLabel(item.labels, item.label, locale);
            var entry = lookupValue(map, item);
            var type = item.type;
            var def_type = entry.type;
            ...
            if ((type === "str") || (type === "num") ||
                ...
                valueField.on("change", function (e, t) {
                    ...
                    map[id] = { name: name, type: type, value: new_val };
                });
                valueField.change();
            }
                ...
                valueField.spinner({
                    "change": function (e, u) {
                        var new_val = valueField.spinner("value");
                ...
                var items = item.items ? item.items : [];
                var labelField = $("<label/>", {
                    style: "width: 25%;"
                }).html(label).appendTo(row0);
                var types = items;
                if (entry) {
                    def_type = entry.value_type;
                }
                ...
                valueField.typedInput("type", def_type);
                valueField.typedInput("value", value);
                valueField.on("change", function (e, t) {
                    var new_val = valueField.typedInput("value");
                ...
                items.forEach(function (elem) {
                    var value = lookupLabel(elem.values, elem.value, locale);
                    val_1st = val_1st ? val_1st : elem.value;
                    $("<option>", {
                        value: elem.value
                ...
                if (value) {
                    ...
                    selector.val(val_1st);
                }
                selector.change(function () {
                    var val = selector.val();
                    map[id] = { name: name, type: type, value: val };
                });
                selector.change();
            }
            else if (type === "label") {
                ...
                console.log("Unexpected parameter type:", type);
            }
        });
    } // buildInputUI

    /**
     * Find start of next ID
     */
    function findNextID(list) {
        var max_id = 0;
        list.each(function (i, item) {
            var entry = $(this);
            var nameField = entry.find(".params-name");
            var id = nameField.data("id");
    ...
    /**
     * Convert UI definition to exportable form
     * @param - list of editable list
     * @returns {object} UI definition
     */
    function exportUISetting(list) {
        var params = [];
        var next_id = findNextID(list);
        list.each(function (i, item) {
            var entry = $(this);
            var nameField = entry.find(".params-name");
            var name = nameField.val();
            var labelField = entry.find(".params-label");
            var labels = labelField.data("labels");
            var label = lookupLabel(labels, labelField.val(), "en-US");
            var ti = entry.find(".params-value");
            var type = ti ? ti.typedInput("type") : null;
            var value = ti ? ti.typedInput("value"): null;
            var id = nameField.data("id");
            if (!id) {
                id = next_id++;
                nameField.data("id", id);
            }
            var result = {
                id: id,
                name: name,
            ...
            if ((type === "str") || (type === "num") ||
                (type === "bool") || (type === "json") ||
                (type === "bin") || (type === "env") ||
                (type === "checkbox") || (type === "spinner")
               ) {
                ; // nothing to do
            }
            else if (type === "menu") {
                var items = entry.find(".menu-item-list").editableList("items");
                var menu_items = [];
                items.each(function (i, item) {
                    var item = $(this).find(".menu-item");
                    var values = item.data("values");
                    var value = lookupLabel(values, item.val(), "en-US");
                    menu_items.push({
                        value: value,
                        values: values
                    });
                ...
                var value_type = ti.typedInput("value");
                var types = entry.find(".type-selector");
                var items = [];
    ...
    /**
     * Load UI definitions to editable list 
     * @param container - editableList
     * @param ui - UI definition
     */
    function loadParams(container, ui) {
        if (ui && ui.params) {
            var params = ui.params;
            for (var i = 0; i < params.length; i++) {
                var param = params[i];
                container.editableList("addItem", param);
            }
        }
    } // loadParams

    /**
     * Copy Env Var definition
     * @param map - original definition
     * @returns {object} copy of env var definition 
     */
    function copyEnvDef(map) {
        var new_map = {};
        if (map) {
    ...
    /**
     * Change Current Locale Setting for UI Editor
     * @param items - list of UI definition items
     * @param locale - new locale
     */
    function changeLocale(items, locale) {
        if (items) {
            items.each(function (i, item) {
                var entry = $(this);
                var labelField = entry.find(".params-label");
                ...
                if (type === "menu") {
                    ...
                    items.each(function (i, item) {
                        var item = $(this).find(".menu-item");
                        var values = item.data("values");
                        var value = lookupLabel(values, item.val(), locale);
                        item.val(value);
                    });
                }
            });
     ...
     * Create UI Edit Form 
     * @param container - enclosing container
     * @param input_con - container for inputting environtment variables
     * @param node - corresponding subflow node
     */
    function buildUiEditForm(container, input_con, node) {
        var ui_in_tmp = copyEnvDef(node.ui_in);
        node.ui_in_tmp = ui_in_tmp;

        var dialogForm = $('<form class="dialog-form form-horizontal" autocomplete="off"></form>').appendTo(container);
        var height = $(window).height() -320;

        // <div></div>
        var top_div = $("<div/>").appendTo(dialogForm);
        
        // UI edit part:
        // <div>
        //     <div id="node-row-ui-edit">
        //         <div id="ui-edit-buttons" class="red-ui-tray-toolbar"/>
        //         <div id="ui-edit-container-row" class="form-row">
        //              <ol id="ui-edit-container"></ol>
        //         </div>
        //     </div>
        // </div>
        var ui_edit = $("<div/>", {
        ...
        var edit_buttons = $("<div/>", {
            "id": "ui-edit-buttons",
            "class": "red-ui-tray-toolbar"
        }).appendTo(ui_edit);
        ...
        }).text(RED._("editor-tab.locale")).appendTo(edit_buttons);
        var locales = $("<select/>", {
            style: "width: 160px; float: left; margin-left: 8px;"
        }).appendTo(edit_buttons);
        ...
        var button_preview = $('<button/>').button().appendTo(edit_buttons);

        var edit_div = $("<div/>", {
            "idname": "ui-edit-container-row",
            "class": "form-row"
        }).appendTo(ui_edit);
        var edit_con = $("<ol/>", {
            "id": "ui-edit-container"
        }).appendTo(edit_div);
        setupUIEditContainer(edit_con, height);
        edit_con.editableList("empty");

        locales.on("change", function() {
            currentLocale = $(this).val();
            var items = edit_con.editableList("items");
            changeLocale(items, currentLocale);
        });

        loadParams(edit_con, node.ui);

        // UI Preview part:
        // <div>
        //     ...
        //     <div id="node-row-ui-preview">
        //         <div id="ui-preview-buttons" class="red-ui-tray-toolbar"/>
        //         <div id="ui-preview-container" class="form-row"/>
        //     </div>
        // </div>
        var ui_preview = $("<div/>", {
            "id": "node-row-ui-preview"
        }).appendTo(top_div);
        var preview_buttons = $("<div/>", {
            "id": "ui-preview-buttons",
            "class": "red-ui-tray-toolbar"
        }).appendTo(ui_preview);
        var button_ok = $('<button/>').button().appendTo(preview_buttons);

        var preview_con = $("<div/>", {
            "id": "ui-preview-container",
            "class": "form-row"
        }).appendTo(ui_preview);

        button_preview.text(RED._("editor-tab.preview"));
        button_preview.click(function(e) {
            e.preventDefault();
            var ui_def = exportUISetting(edit_con.editableList("items"));
            buildInputUI(preview_con, ui_def, {}, currentLocale);
            ui_edit.hide();
            ui_preview.show();
        });

        button_ok.text(RED._("common.label.ok"));
        ...
        if (input_con) {
            var ui = node.ui;
            if (!ui || !ui.params || (ui.params.length === 0)) {
                var sfid = node.type.substring(8);
                var sf = RED.nodes.subflow(sfid);
                if (sf) {
                    ui = sf.ui;
                }
            }
            var editorLocale = getEditorLocale();
            buildInputUI(input_con, ui, ui_in_tmp, editorLocale);
        }

    } // buildUiEditForm

    /**
     * Create Form for Inputting Environment Variable using UI
     * @param - enclosing container
     */
    function buildUiInputForm(container) {
        var dialogForm = $('<form class="dialog-form form-horizontal" autocomplete="off"></form>').appendTo(container);
        var div = $("<div>", {
            class: "form-row",
            id: "ui-input-container"
        }).appendTo(dialogForm);
        return div;
    } // buildUiInputForm
    ...
    function showEditDialog(node) {
        ...
        var trayOptions = {
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (type === "subflow") {
                            ...
                            // UI definitions
                            var old_ui = editing_node.ui;
                            var new_ui = exportUISetting($("#ui-edit-container").editableList("items"));
                            var changed_ui_def = !isSameObj(old_ui, new_ui);
                            if (changed_ui_def) {
                                editing_node.ui = new_ui;
                                changes.ui = editing_node.ui;
                                changed = true;
                            }
                            // UI inputs
                            var old_ui_in = editing_node.ui_in;
                            var new_ui_in = editing_node.ui_in_tmp;
                            if (!isSameObj(old_ui_in, new_ui_in)) {
                                editing_node.ui_in = new_ui_in;
                                editing_node.ui_in_tmp = null;
                                changes.ui_in = editing_node.ui_in;
                                changed = true;
                            }
                         }
            ...
            open: function(tray, done) {
                ...
                if ((type === "subflow") || (type === "subflow-template")) {
                    var uiEditTab = {
                        id: "editor-tab-ui-edit",
                        label: RED._("editor-tab.ui-edit"),
                        name: RED._("editor-tab.ui-edit"),
                        content: $('<div>', {class:"red-ui-tray-content"}).appendTo(editorContent).hide(),
                        iconClass: "fa fa-desktop",
                        onchange: function() {
                        }
                    };
                    editorTabs.addTab(uiEditTab);

                    var uiInputTab = null;
                    if (type === "subflow") {
                        uiInputTab = {
                            id: "editor-tab-ui-input",
                            label: RED._("editor-tab.ui-input"),
                            name: RED._("editor-tab.ui-input"),
                            content: $('<div>', {class:"red-ui-tray-content"}).appendTo(editorContent).hide(),
                            iconClass: "fa fa-list-alt",
                            onchange: function() {
                            }
                        };
                        editorTabs.addTab(uiInputTab);
                    }
                    
                    var input_con = uiInputTab ? buildUiInputForm(uiInputTab.content) : null;
                    buildUiEditForm(uiEditTab.content, input_con, node);
                    if ((type === "subflow") && hasUI(node)) {
                        // if subflow node has UI defined, make input tab
                        // selected by default
                        editorTabs.activateTab("editor-tab-ui-input");
                    }
                }
        ...
        var trayOptions = {
            ...
            open: function(tray) {
                ...
                if (type === "subflow") {
                    ...
                    editorTabs.addTab(uiEditTab);
                    buildUiEditForm(uiEditTab.content, null, editing_node);
                }
