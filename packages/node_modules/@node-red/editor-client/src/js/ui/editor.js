    ...
    function createMonacoEditor(options) {
        ...
        if(editorOptions.extraLibs && Array.isArray(editorOptions.extraLibs)) {
            ...
            for (let index = 0; index < editorOptions.extraLibs.length; index++) {
                ...
                if(varName && moduleName) defs.push(`const ${varName} = require('${moduleName}');\n`)
                //if(varName && moduleName) defs.push(`import * as ${varName} from '${moduleName}';\n`)
                /*
                    TODO: import * as os from 'os';  AND const os = require("os") 
                    both provide intelisense in the monaco editor however when added as an `addExtraLib`
                    `os` either doesnt exist or intellisense doesnt work!
                    for now, setting up the var as a const with jsdoc typeing at least lets the editor know it exists
                */
                //if(varName && moduleName) defs.push(`/** @type {${moduleName}} */ const ${varName};\n`);
                //if(varName && moduleName) defs.push(`/** @type {${moduleName}} */ const ${varName}:${moduleName};\n`);
            }
            if(defs.length) {
                var definitions = defs.join("");
                monaco.languages.typescript.javascriptDefaults.addExtraLib(definitions, "ts:extraLibs.d.ts");
                monaco.languages.typescript.typescriptDefaults.addExtraLib(definitions, "ts:extraLibs.d.ts");
            }
        }
