...
RED.userSettings = (function () {
    ...
    function createAIRewriterPane() {
        var pane = $('<div id="red-ui-settings-tab-ai-rewriter" class="red-ui-help" ></div>');

        $('<h3  style="padding:0px 10px;" ></h3>').text("AI Rewriter").appendTo(pane);

        var row = $('<div class="red-ui-settings-row"></div>').appendTo(pane);
        $('<label for="user-settings-ai-rewriter-openai-api-key"></label>')
            .text("OpenAI API key")
            .appendTo(row);

        $('<input id="user-settings-ai-rewriter-openai-api-key" type="password" autocomplete="off" placeholder="Enter OpenAI API key">')
            .appendTo(row)
            .val(RED.settings.get("aiRewriter.openaiApiKey", ""));
    ...
    function setViewDarkTheme(val) {
        if (!['auto', 'light', 'dark'].includes(val)) { return; }
        if (!allSettings['view-dark-theme']) { return; }
        setSelected('view-dark-theme', val);
        $("#user-settings-view-dark-theme").val(val);
    }

    function init() {
        RED.actions.add("core:show-user-settings", show);
        RED.actions.add("core:show-keyboard-settings", function () { show('keyboard') });
        ...
        addPane({
            id: 'ai-rewriter',
            title: "AI Rewriter",
            get: createAIRewriterPane,
            close: function () {
                RED.settings.set(
                    "aiRewriter.openaiApiKey",
                    $("#user-settings-ai-rewriter-openai-api-key").val()
                );
            }
        })
        ...
        if (window.parent !== window) {
            window.addEventListener('message', function (event) {
                if (event.data && event.data.type === 'set-theme') {
                    setViewDarkTheme(event.data.payload && event.data.payload.theme);
                }
            });
