(function () {
    ...
    // get the correct cache buster value from the script src
    const scriptSrc = bootstrapEl.getAttribute('src') || document.currentScript.src
    let appendCacheBuster = ''
    if (scriptSrc.indexOf('?') > -1) {
        const urlParams = new URLSearchParams(scriptSrc.split('?')[1])
        const cacheBuster = urlParams.get('v') || ''
        appendCacheBuster = cacheBuster ? '?v=' + cacheBuster : ''
    }
    ...
    if (langPack && langPack !== 'BUILT-IN') {
        ...
        langScriptEl.src = 'vendor/monaco/dist/locale/' + langPack + appendCacheBuster
        langScriptEl.async = true
        langScriptEl.onload = loadEditor
    ...
    function loadEditor() {
        // load vendor/monaco/dist/editor.js
        const editorScriptEl = document.createElement('script')
        editorScriptEl.src = 'vendor/monaco/dist/editor.js' + appendCacheBuster
