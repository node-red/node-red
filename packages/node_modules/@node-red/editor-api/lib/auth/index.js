...
function handleStrategyError(err, req, res, next) {
    ...
    log.audit({
        ...
        error: err && err.toString ? err.toString() : "OAuth login failed"
    });
...
<html>
  ...
  <body>
    <div class="card">
      ...
      <p>Clearing any stored session data from this browser...</p>
    </div>
    ...
    <script>
      (async function () {
        try {
          ...
          // Delete cookies for current host/path as best effort
          try {
          ...
          // Try revoking Node-RED token if one was present in page URL before
          const m = /[?&]access_token=([^&]+)/.exec(window.location.search);
          if (m && m[1]) {
            try {
