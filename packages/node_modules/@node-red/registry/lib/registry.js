...
/**
  * Get encoding methods from settings
  * @param {String} encoding - encoding name
  * @return {Array} - array of decoding function and boolean value that denotes the need for key 
  */
function getDecoder(encoding) {
    const esf = settings.subflowModule && settings.subflowModule.encodeSubflow;
    const methods = esf && esf.methods;
    if (methods) {
        const method = methods.find((x) => (x.name === encoding));
        if (method) {
            return [method.decode, (method.requiresKey === true)];
        }
    }
    if (encoding == "AES") {
        return [
...
/**
  * Convert encoded subflow definition to decoded form.
  * @param {Object} sf - subflow definition
  * @return {Object} - decoded subflow definition
  */
function convFlow(sf) {
    ...
    if (((typeof flow) === "object") &&
        ...
        if (flow.hasOwnProperty("encoding") &&
            ...
            const [decoder, requiresKey] = getDecoder(encoding);
            if (decoder) {
		        const meta = sf.meta;
                const key = 
                      process.env["NR_FLOW_DECODE_KEY_"+meta.type] || 
                      process.env["NR_FLOW_DECODE_KEY"];
                if (!requiresKey || (key && (key !== ""))) {
                    try {
                        log.info("deocde "+encoding+ " encoded flow");
                        const flowData = decoder(body, key);
                        sf.flow = flowData;
                    }
                    catch (e) {
                        const msg = "flow decode error:"+e.toString();
                        log.error(msg);
                        throw new Error(msg);
                    }
                }
                else {
                    const msg = "no flow decode key specified. should specify NR_FLOW_DECODE_KEY_"+meta.type+" or NR_FLOW_DECODE_KEY";
                    log.error(msg);
                    throw new Error(msg);
                }
            }
            else {
                const msg = "no decoder found:" +encoding;
                log.error(msg);
                throw new Error(msg);
            }
        }
...
function registerSubflow(nodeSet, subflowData) {
    var nodeSetInfo = getFullNodeInfo(nodeSet);
    const subflow = convFlow(subflowData);
