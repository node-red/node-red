...
Node.prototype._emitInput = function(arg) {
    ...
    if (node._inputCallback) {
        ...
        try {
            if(this._wireCount === 0 && !this._wire) {
                timestamp = new Date().getTime();
                stopFlowDetails(timestamp,this._flow.flow);
            }
            node._inputCallback(
                arg,
...
flowLifecycleEvent = (data) => {
    if(data.event_type === "start") {
        executionEndArray = [];
        console.log(data);
    }
    if(data.event_type === "stop") {
        console.log(data);
        executionStartArray = [];
    }
...
var executionStartArray = [];
startFlowDetails = (timestamp, data) => {
    req_data = {
        flow_id : data.id,
        event_type : "start", 
        timestamp : timestamp
    }
    executionStartArray.push(req_data);
    if(executionStartArray.length === 1) {
        flowLifecycleEvent(executionStartArray[0]);
    }
}

var executionEndArray = [];
stopFlowDetails = (timestamp, data) => {
    req_data = {
        flow_id : data.id,
        event_type : "stop", 
        timestamp : timestamp  
    }
    executionEndArray.unshift(req_data);
    if(executionEndArray.length === 1) {
        flowLifecycleEvent(executionEndArray[0]);
    }
}

errorFlowDetails = (msg, data) => {
     req_data = {
        flow_id:data.z,
        event_type: "error",
        error_node: data.name ? data.name : data.type,
        message: msg
    };
    flowLifecycleEvent(req_data);
}

Node.prototype.send = function(msg) {
    timestamp = new Date().getTime();
    startFlowDetails(timestamp, this._flow.flow);
    var msgSent = false;
    var node;
...
Node.prototype.error = function(logMessage,msg) {
    errorFlowDetails(logMessage, this);
