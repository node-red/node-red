...
class Subflow extends Flow {
    ...
    constructor(parent,globalFlow,subflowDef,subflowInstance) {
        ...
        this.env = env;
        var ui = [];
        if (this.subflowDef.ui) {
            ui = deepCopy(this.subflowDef.ui);
            ui.forEach(item => {
                var name = item.name;
                env[name] = item;
            });
        }
        if (this.subflowInstance.ui) {
            ui = deepCopy(this.subflowInstance.ui);
            ui.forEach(item => {
                var name = item.name;
                env[name] = item;
            });
        }
        var ui_in = this.subflowInstance.ui_in;
        if (ui_in) {
            Object.keys(ui_in).forEach(name => {
                var item0 = env[name];
                var item1 = ui_in[name];
                if (item0 && item1 && item1.hasOwnProperty("value")) {
    ...
    getSetting(name) {
        ...
        var ename = is_info ? name.substring(0, name.length -5) : name; // 5 = length of "_info"

        if (env && env.hasOwnProperty(ename)) {
            var val = env[ename];
            if ((val.type === "checkbox") || (val.type === "menu")) {
                if (is_info) {
                    return composeInfo(val, val.value);
                }
                return val.value;
            }
            else {
