...
class Flow {
    ...
    constructor(parent,globalFlow,flow) {
        ...
        this.catchNodes = [];
        this.statusNodes = [];
        this.pendingMsgIds = new Map();
        this.shutdownBehaviour = 'whenReady'; // or 'selectedNodesFirst'
    }
    ...
    stop(stopList, removedList) {
        ...
        let spoutPromises = [];
        let waitPending = Promise.resolve();
        
        if (gracefulShutdown) {
            if (!this.areNodesSupportsMessagingAPI()) {
                this.log("Graceful shutdown may fail because some node does not support Node Messaging API.");
            }

            let spoutNodes = this.flow.spoutNodes || [];
            let shutdownBehaviour = this.flow.shutdownBehaviour;

            if (shutdownBehaviour === 'selectedNodeFirst') {
                for (i = 0; i < spoutNodes.length; i++) {
                    const node = this.activeNodes[spoutNodes[i]];
                    if (node) {
                        delete this.activeNodes[spoutNodes[i]];
                        try {
                            const removed = removedMap[spoutNodes[i]];
                            spoutPromises.push(stopNode(node, removed).catch(() => { }));
                        } catch (err) {
                            node.error(err);
                        }
                    }
            ...
            this.trace(`Graceful period = ${gracefulPeriod}`);
            waitPending = new Promise((resolve, reject) => {
                setTimeout(() => resolve(), gracefulPeriod);
                const loop = () => {
                    this.trace(`number of in-progress processes: ${this.pendingMsgIds.size}`);
                    if (this.pendingMsgIds.size === 0) {
                        return resolve()
                    }
                    setTimeout(loop, 1000); // polling every 100ms
                }
                setTimeout(loop, 0);
            });
        }
        ...
        return Promise.all(spoutPromises).then(()=>waitPending).then(finalclosing);
    }
    ...
    handleComplete(node,msg) {
        if (this.completeNodeMap[node.id]) {
            ...
            this.completeNodeMap[node.id].forEach((completeNode,index) => {
                ...
                this.incrPendingMsgId(toSend._msgid);
                completeNode.receive(toSend);
            })
        }
        this.decrPendingMsgId(msg._msgid);
    }
    ...
    incrPendingMsgId(msgid) {
        if (this.pendingMsgIds.has(msgid)) {
            this.pendingMsgIds.set(msgid, this.pendingMsgIds.get(msgid) + 1);
        } else {
            this.pendingMsgIds.set(msgid, 1);
        }
        this.trace(`incrPendingMsgId(${msgid}): ${this.pendingMsgIds.get(msgid)}`);
    }

    decrPendingMsgId(msgid) {
        this.trace(`decrPendingMsgId(${msgid}): ${this.pendingMsgIds.get(msgid)}`);
        if (this.pendingMsgIds.has(msgid)) {
            let curval = this.pendingMsgIds.get(msgid);
            if (curval > 1) {
                this.pendingMsgIds.set(msgid, curval - 1);
            } else {
                this.pendingMsgIds.delete(msgid);
            }
            this.trace(`        -> ${this.pendingMsgIds.get(msgid)}`);
            return curval - 1;
        }
        return -1;
    }

    areNodesSupportsMessagingAPI() {
        const activeNodes = Object.values(this.getActiveNodes());
        return activeNodes.every(n => {
            if (n._inputCallbacks) {
                return n._inputCallbacks.every(cb => cb.length === 3); // 3 => msg, send, done
            } else if (n._inputCallback) {
                return n._inputCallback.length === 3;
            } else {
                return true;
            }
        });
...
module.exports = {
    init: function(runtime) {
        ...
        Log = runtime.log;
        gracefulShutdown = runtime.settings.gracefulShutdown || false;
        gracefulPeriod = gracefulShutdown ? (runtime.settings.gracefulPeriod || 10000) : 0;
