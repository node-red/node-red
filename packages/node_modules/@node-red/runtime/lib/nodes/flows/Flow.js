...
class Flow {
    ...
    stop(stopList, removedList) {
        ...
        if (gracefulShutdown) {
            ...
            let starterNodes = this.flow.starterNodes || [];

            for (i = 0; i < starterNodes.length; i++) {
                const node = this.activeNodes[starterNodes[i]];
                if (node) {
                    delete this.activeNodes[starterNodes[i]];
                    try {
                        const removed = removedMap[starterNodes[i]];
                        starterPromises.push(stopNode(node, removed).catch(() => {}));
                    } catch (err) {
                        node.error(err);
                    }
                }
        ...
        return Promise.all(starterPromises).then(()=>waitPending).then(finalclosing);
    }
    ...
    areNodesSupportsMessagingAPI() {
        ...
        return activeNodes.every(n => {
            if (n._inputCallbacks) {
                return n.inputCallbacks.every(cb => cb.length === 3); // 3 => msg, send, done
