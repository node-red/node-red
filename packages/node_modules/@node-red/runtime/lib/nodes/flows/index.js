...
function stop(type,diff,muteLog) {
    ...
    var gracefulShutdown = settings.gracefulShutdown || false;
    var gracefulPeriod = gracefulShutdown ? (settings.gracefulPeriod || 10000) : 0;

    const gracefulPromises = [];
    for (var id in activeFlows) {
        if (activeFlows.hasOwnProperty(id) && id != 'global') {
            const starterNodes = getFlow(id).starterNodes;
            if (starterNodes && starterNodes.length > 0) { 
                log.debug("red/nodes/flows.stop : stopping starter nodes in flow : " + id);
                gracefulPromises.push(activeFlows[id].stop(starterNodes,null));
                if (stopList) {
                    stopList = stopList.filter(e => !starterNodes.includes(e));
                }
            }
        }
    }
    if (gracefulPromises.length == 0) {
        gracefulPeriod = 0;
    }
    const waitInProgress = ms => {  // TODO: use Node Messaging & Timeout APIs so that the runtime detects completion of in-progress processes
        log.trace(`graceful period: ${ms} ms`);
        return new Promise(resolve => setTimeout(resolve, ms));
    };

    const finalCloses = function() {
        for (var id in activeFlows) {
            if (activeFlows.hasOwnProperty(id)) {
    ...
    return Promise.allSettled(gracefulPromises)
        .then(() => waitInProgress(gracefulPeriod))
        .then(finalCloses)
        .then(cleanup);
}
...
function addFlow(flow) {
    ...
    if (flow.hasOwnProperty('starterNodes')) {
        tabNode.starterNodes = flow.starterNodes;
    }
...
function getFlow(id) {
    ...
    if (flow.hasOwnProperty('starterNodes')) {
        result.starterNodes = flow.starterNodes;
    }
    if (id !== 'global') {
        result.nodes = [];
    }
...
function updateFlow(id,newFlow) {
    ...
    if (id === 'global') {
        ...
        if (newFlow.hasOwnProperty('starterNodes')) {
            tabNode.starterNodes = newFlow.starterNodes;
        }
