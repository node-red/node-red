...
class Flow {
    ...
    stop(stopList, removedList) {
        ...
        if (gracefulShutdown && this.TYPE === "flow") {
            if (!this.areNodesSupportsMessagingAPI()) {
                this.info("Graceful shutdown may fail because some node does not support Node Messaging API.");
            }
            this.trace(`number of in-progress processes: ${this.getPendingMsgNums()}`);
            ...
            waitPending = new Promise((resolve, reject) => {
                let remainingPeriod = gracefulPeriod;
                const loop = () => {
                    this.trace(`number of in-progress processes: ${this.getPendingMsgNums()}`);
                    if (this.getPendingMsgNums() <= 0 || remainingPeriod < 0) {
                        this.resetPendingMsgNums();
                        return resolve();
                    }
        ...
        return Promise.all(spoutPromises)
                .then(()=>waitPending)
    ...
    handleComplete(node,msg) {
        if (this.completeNodeMap[node.id]) {
            ...
            this.completeNodeMap[node.id].forEach((completeNode,index) => {
                ...
                this.incrPendingMsgNums();
                completeNode.receive(toSend);
            })
        }
        this.decrPendingMsgNums();
    }
