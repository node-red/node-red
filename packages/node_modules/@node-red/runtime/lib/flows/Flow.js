...
class Flow {
    ...
    handleStatus(node,statusMessage,reportingNode,muteStatusEvent) {
        ...
        if (this.statusNodeTimers[node.id]) {
            // Cancels the previous timeout to clear the node status
            clearTimeout(this.statusNodeTimers[node.id]);
            delete this.statusNodeTimers[node.id];
        }

        let handled = false;
        if (statusMessage.hasOwnProperty("duration") && !statusMessage.ephemeral) {
            const duration = parseInt(statusMessage.duration, 10);
            if (duration && !isNaN(duration)) {
                this.statusNodeTimers[node.id] = setTimeout(() => {
                    // Set a timeout to clear the node status
                    handled = this.handleStatus(node, {}, reportingNode, true) || handled;
                }, duration);
            }
        }

        if (statusMessage.hasOwnProperty("ephemeral") && statusMessage.ephemeral === true) {
            // Do not trigger Status nodes if the status is ephemeral
            return true;
        }
