...
/**
 ** Add local subflow information to subflow definition.
 * @param {Object} def       subflow definition
 */
function addSubflow(def) {
    const flow = def.flow;
    const subflows = def.subflows || {};
    def.subflows = subflows;
    flow.forEach((n) => {
        if (n.type === "subflow") {
            subflows[n.id] = n;
            n.configs = {};
