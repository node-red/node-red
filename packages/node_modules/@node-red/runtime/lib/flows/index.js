...
function setFlows(_config,_credentials,type,muteLog,forceStart,user) {
    ...
    var newFlowConfig;
    var isLoad = false;
    var dup = isJSONType(_config) ? rfdcClone : clone;
    if (type === "load") {
        isLoad = true;
        configSavePromise = loadFlows().then(function(_config) {
            config = dup(_config.flows);
            newFlowConfig = flowUtil.parseConfig(dup(config));
            type = "full";
            return _config.rev;
        });
    } else {
...
async function addFlow(flow, user) {
    ...
    var newConfig = isJSONType(activeConfig.flows) ? rfdcClone(activeConfig.flows) : clone(activeConfig.flows);
...
function getFlow(id) {
    ...
    if (flow.nodes) {
        ...
        if (nodeIds.length > 0) {
            result.nodes = nodeIds.map(function(nodeId) {
                var dup = isJSONType(flow.nodes[nodeId]) ? rfdcClone : clone;
                var node = dup(flow.nodes[nodeId]);
    ...
    if (flow.configs) {
        ...
        result.configs = configIds.map(function(configId) {
            var dup = isJSONType(flow.configs[configId]) ? rfdcClone : clone;
            const node = dup(flow.configs[configId]);
            delete node.credentials;
            return node
    ...
    if (flow.subflows) {
        ...
        result.subflows = subflowIds.map(function(subflowId) {
            var dup = isJSONType(flow.subflows[subflowId]) ? rfdcClone : clone;
            var subflow = dup(flow.subflows[subflowId]);
            var nodeIds = Object.keys(subflow.nodes);
            subflow.nodes = nodeIds.map(function(id) {
                var dup = isJSONType(subflow.nodes[id]) ? rfdcClone : clone;
                const node = dup(subflow.nodes[id])
                delete node.credentials
                return node
            });
            if (subflow.configs) {
                ...
                subflow.configs = configIds.map(function(id) {
                    var dup = isJSONType(subflow.configs[id]) ? rfdcClone : clone;
                    const node = dup(subflow.configs[id])
                    delete node.credentials
                    return node
                })
            }
