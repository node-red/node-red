...
var storageModuleInterface = {
        ...
        // A storage module declares which project type versions its flows (e.g.
        // localfilesystem -> "git") and supplies the project layout that type uses.
        getProjectType: function() {
            return storageModule?.getProjectType ? storageModule.getProjectType() : undefined;
        },
        getProjectLayout: function() {
            return storageModule?.getProjectLayout ? storageModule.getProjectLayout() : undefined;
        },
        getFlows: async function() {
            // Project-state admission (merge conflict, empty repo, etc.) is a
            // runtime/projects policy, not a storage-format concern — gate it here
            // at the runtime edge. No-op when no project type is active.
            if (runtime.projects) {
                runtime.projects.assertCanLoadFlows();
            }

            var flows = await storageModule.getFlows();
            var creds = await storageModule.getCredentials();
            var rev = crypto.createHash('sha256').update(JSON.stringify(flows)).digest("hex");
            return {
                flows: flows,
                credentials: creds,
                rev: rev
        ...
        saveFlows: async function(config, user) {
            // If we're not going to persist anything (read-only), there is nothing
            // to gate — the storage module no-ops the write itself.
            if (!runtime.settings.readOnly && runtime.projects) {
                runtime.projects.assertCanSaveFlows();
            }

            if (config.credentialsDirty) {
                await storageModule.saveCredentials(config.credentials);
            }
            delete config.credentialsDirty;

            await storageModule.saveFlows(config.flows, user);
            if (runtime.projects) {
                await runtime.projects.handleFlowFileSave(user);
            }
