...
var api = module.exports = {
    ...
    */
    getRuntimeSettings: async function(opts) {
        ...
        if (!runtime.settings.disableEditor) {
            ...
            if (runtime.projects && runtime.projects.available()) {
                var activeProject = runtime.projects.getActiveProject();
                if (activeProject) {
                    safeSettings.project = activeProject.export ? activeProject.export() : activeProject;
                } else if (runtime.projects.flowFileExists()) {
                    // The editor reads RED.settings.files.{flow,credentials} by name.
                    safeSettings.files = runtime.projects.getDefaultProjectFilesForEditor();
                }
                safeSettings.git = {
                    globalUser: runtime.projects.getGlobalGitUser()
                }
            }
    ...
    */
    getUserKeys: async function(opts) {
        ...
        return runtime.projects.ssh.listSSHKeys(username).catch(function(err) {
            err.status = 400;
            throw err;
            return reject(err);
        });
    ...
    */
    getUserKey: async function(opts) {
        ...
        return runtime.projects.ssh.getSSHKey(username, opts.id).then(function(data) {
            if (data) {
                return data;
            } else {
    ...
    */
    generateUserKey: async function(opts) {
        var username = getSSHKeyUsername(opts.user);
        return runtime.projects.ssh.generateSSHKey(username, opts).catch(function(err) {
            err.status = 400;
            throw err;
        });
    },
    ...
    */
    removeUserKey: async function(opts) {
        var username = getSSHKeyUsername(opts.user);
        return runtime.projects.ssh.deleteSSHKey(username, opts.id).catch(function(err) {
            err.status = 400;
            throw err;
        });
    }
