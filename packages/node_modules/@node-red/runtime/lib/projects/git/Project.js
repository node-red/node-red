...
var util = require("../../storage/localfilesystem/util");
...
// Render a default repo file. For package.json, the git type builds the
// skeleton and lets the layout record its flow file references (decision A).
function buildDefaultFile(file, project) {
    var content = defaultFileSet[file](project);
    if (file === 'package.json' && project.files) {
        var pkg = JSON.parse(content);
        layout.buildPackageSettings(project, pkg['node-red'].settings, project.files);
...
// console.log(globalProjectSettings)
    var projectSettings = {};
    var allProjects = globalProjectSettings.projects || {};
    if (allProjects.hasOwnProperty(this.name)) {
        projectSettings = allProjects[this.name] || {};
    }
    ...
    return checkProjectFiles(project).then(function(missingFiles) {
        ...
        if (missingFiles.indexOf('package.json') === -1) {
            ...
            promises.push(fs.readFile(fspath.join(project.path,project.paths['package.json']),"utf8").then(function(content) {
                try {
                    // The git type holds only repo metadata; which files hold the
                    // flows/credentials is resolved on demand by the storage layout.
...
Project.prototype.initialise = function(user,data) {
    ...
    if (data.hasOwnProperty('files')) {
        ...
        var created = layout.createFiles(project);
        if (created.files) {
            files.push.apply(files, created.files);
        }
        if (created.write) {
            promises.push(created.write);
        }
    }
    for (var file in defaultFileSet) {
        if (defaultFileSet.hasOwnProperty(file)) {
            ...
            if (!fs.existsSync(path)) {
                promises.push(util.writeFile(path,buildDefaultFile(file,project)));
            }
...
Project.prototype.update = async function (user, data) {
    ...
    if (data.hasOwnProperty('files')) {
        ...
        if (data.files.hasOwnProperty('package') && (data.files.package !== fspath.join(this.paths.root,"package.json") || !this.paths['package.json'])) {
            ...
            if (!/package\.json$/.test(data.files.package)) {
                throw new Error("Invalid package file: "+data.files.package)
            }
        ...
        // The layout records a flow/credentials rename into the project manifest.
        // {files, write} are for open ended project update extensibility.
        var renamed = layout.updateFiles(this, data.files);
        if (renamed.files) {
            modifiedFiles.push.apply(modifiedFiles, renamed.files);
        }
        if (renamed.write) {
            promises.push(renamed.write);
        }
        if (renamed.changed) {
            savePackage = true;
            flowFilesChanged = true;
    ...
    return Promise.all(promises).then(function(res) {
        ...
        if (workflowMode === 'auto') {
            return project.stageFile(modifiedFiles.map(f => project.paths[f] || f)).then(() => {
...
Project.prototype.listAutosaveFiles = function() {
    // Repo-level files (package.json, README.md) are owned by the git type; the
    // flow data files come from the storage layout. layout.listAutosaveFiles returns
    // an arbitrary-length list, so this works for a single-file layout or a
    // multi-file one without the git type knowing the format.
    return layout.listAutosaveFiles(this).filter(Boolean);
}
...
Project.prototype.export = function () {
    return {
        ...
        files: Object.assign(
            ...
            layout.exportFilesForEditor(this)
        ),
        git: {
...
function createDefaultProject(user, project) {
    ...
    project.path = projectPath; // ?? is this okay?
    // Create a basic skeleton of a project
    return gitTools.initRepo(projectPath).then(function() {
        var promises = [];
        var files = Object.keys(defaultFileSet);
        if (project.migrateFiles) {
            // The runtime exports the live credentials and hands them in via
            // metadata; the layout writes the migrated flow + credentials.
            var migrated = layout.migrateFiles(project);
            if (migrated.files) {
                files.push.apply(files, migrated.files);
            }
            if (migrated.write) {
                promises.push(migrated.write);
            }
        ...
        for (var file in defaultFileSet) {
            if (defaultFileSet.hasOwnProperty(file)) {
                promises.push(util.writeFile(fspath.join(projectPath,file),buildDefaultFile(file,project)));
            }
        }
