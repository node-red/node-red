...
module.exports = function(RED) {
    ...
    function SwitchNode(n) {
        ...
        function addMessageToGroup(id, msgInfo, parts) {
            if (!(id in pendingIn)) {
                pendingIn[id] = {
                    count: undefined,
                    msgs: [],
            ...
            if ((max_msgs > 0) && (pendingCount > max_msgs)) {
                clearPending();
                node.error(RED._("switch.errors.too-many"), msgInfo.msg);
            }
        ...
        function drainMessageGroup(msgs,count,done) {
            ...
            processMessage(msgInfo,false, err => {
                if (err) {
                    done(err);
                } else {
                    if (msgs.length === 0) {
        ...
        function addMessageToPending(msgInfo,done) {
            const msg = msgInfo.msg;
            var parts = msg.parts;
            // We've already checked the msg.parts has the require bits
            var group = addMessageToGroup(parts.id, msgInfo, parts);
            var msgs = group.msgs;
        ...
        function sendGroup(onwards, port_count, send) { 
        ...
        function sendGroupMessages(onward, msg, send) {
            var parts = msg.parts;
            var gid = parts.id;
            ...
            pendingCount++;
            if (send_ok) {
                sendGroup(onwards, onward.length, send);
        ...
        function processMessage(msgInfo, checkParts, done) {
            const msg = msgInfo.msg;
            ...
            if (needsCount && checkParts && hasParts) {
                addMessageToPending(msgInfo,done);
            } else {
                        ...
                        applyRules(node,msg,property,undefined,(err,onward) => {
                            if (err) {
                                ...
                                if (!repair || !hasParts) {
                                    ...
                                    sendGroupMessages(onward, msg, msgInfo.send);
                                }
                                msgInfo.done();
                            }
                            done();
        ...
        var pendingMessages = [];  // KT: [{msg, send, done}]
        var handlingMessage = false;
        var processMessageQueue = function(msg, send, done) {
            if (msg) {
