...
<script type="text/html" data-template-name="trigger">
    ...
    <div class="form-row">
        ...
        <input type="hidden" id="node-input-triggerfunction">
    </div>
...
<script type="text/javascript">
    RED.nodes.registerType('trigger',{
        ...
        icon: "trigger.svg",
        label: function() {
            if (this.triggerfunction === "wait") {
                return this.name|| this._("trigger.label.trigger")+" "+this.duration+this.units;
            }
            if (this.triggerfunction === "loop") {
                return this.name|| this._("trigger.label.trigger-loop")+" "+(this.duration)+this.units;
            }
            else {
        ...
        oneditprepare: function() {
            $("#node-then-type").on("change", function() {
                if ($(this).val() == "block") {
                    ...
                    $("#node-input-triggerfunction").val("block");
                }
                else if ($(this).val() == "loop") {
                    if ($("#node-input-duration").val() == 0) { $("#node-input-duration").val(250); }
                    $(".node-type-wait").hide();
                    $(".node-type-duration").show();
                    $("#node-input-triggerfunction").val("loop");
                } else {
                    if ($("#node-input-duration").val() == 0) { $("#node-input-duration").val(250); }
                    $(".node-type-wait").show();
                    $(".node-type-duration").show();
                    $("#node-input-triggerfunction").val("wait");
                }
            });
            ...
            if (this.triggerfunction === "block" || this.duration == "0") {
                $("#node-then-type").val("block");
            }
            else if (this.triggerfunction === "loop" || this.duration < 0) {
                if (this.duration < 0)
                {
        ...
        oneditsave: function() {
            ...
            else if ($("#node-then-type").val() == "loop") {
                $("#node-input-triggerfunction").val("loop");
            }
            else {
                $("#node-input-triggerfunction").val("wait");
