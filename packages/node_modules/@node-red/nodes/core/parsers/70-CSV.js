...
module.exports = function(RED) {
    ...
    function CSVNode(n) {
        ...
        this.parse_empty_strings = n.parse_empty_strings || false;
        this.parse_null_values = n.parse_null_values || false;
                            ...
                            if ((node.hdrin === true) && first) { // if the template is in the first line
                                ...
                                if (line[i] === node.quo) { // if it's a quote toggle inside or outside
                                    ...
                                    if ( node.template[j] && (node.template[j] !== "") ) {
                                        ...
                                        //if (node.parse_null_values && (line[i-1] === node.sep || line[i-1].includes('\n','\r'))) k[j] = null;
                                        if (line[i-1] === node.sep || line[i-1].includes('\n','\r')) k[j] = null;
                                        if ( (k[j] !== null && node.parsestrings === true) && reg.test(k[j]) ) { k[j] = parseFloat(k[j]); }
                                        if (node.parse_null_values && k[j] === null) o[node.template[j]] = k[j];
                                        if (node.parse_empty_strings && k[j] === "") o[node.template[j]] = k[j];
                                        if (k[j] !== null && k[j] !== "") o[node.template[j]] = k[j];
                                    }
                                    j += 1;
                                    // if separator is last char in processing string (without end of line), add null value at the end ie. '1,2,3\n3,"3",'
