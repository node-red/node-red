    ...
    <div class="form-row">
        ...
        <div style="margin-left: 20px" class="node-input-useAuth-row hide">
            ...
            <div class="form-row node-input-bearer-row hide">
                ...
                <input type="password" id="node-input-bearerToken">
            </div>
        </div>
    </div>
        ...
        debugger
            ...
            debugger;
                if ($(this).is(":checked")) {
                    ...
                    if ($('#node-input-authType').val() == 'none') {
                        $('#node-input-authType').val('basic');
                        $("#node-input-authType").change();
                    }
                } else {
                    $(".node-input-useAuth-row").hide();
                    // Set authType to 'none' instead of undefined, because undefined is converted to 
                    // 'basic' on the server-side (for backwards compatibility with older flows).
                    $('#node-input-authType').val('none');
                    $('#node-input-user').val('');
                    $('#node-input-password').val('');
                    $('#node-input-bearerToken').val('');
                }
            });
            $("#node-input-authType").change(function() {
                if ($(this).val() == "basic" || $(this).val() == "digest") {
                    $(".node-input-basic-row").show();
                    $(".node-input-bearer-row").hide();
                    $('#node-input-bearerToken').val('');
                } else if ($(this).val() == "bearer") {
                    $(".node-input-basic-row").hide();
                    $(".node-input-bearer-row").show();
                    $('#node-input-user').val('');
                    $('#node-input-password').val('');
                } else { // "none"
                    $(".node-input-basic-row").hide();
            ...
            if (this.credentials.user || this.credentials.has_password || this.credentials.has_bearerToken) {
