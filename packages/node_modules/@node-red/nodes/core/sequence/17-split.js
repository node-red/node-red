    ...
    function JoinNode(n) {
        ...
        this.on("input", function(msg) {
            try {
                ...
                if (node.mode === "auto") {
                ...
                else if(node.mode==='once') {
                    var msgId;
                    var contextKey = RED.util.parseContextStore('memorize');
                    var target = node.context()['flow'];
                    target.get(contextKey.key,contextKey.store,(err,value) => {
                        if(err) {
                            done(err);
                        } else {
                            msgId = value;
                    ...
                    if(msgId === msg._msgid) {
                        target.set(contextKey.key, "", contextKey.store, function (err) {
                            if(err) {
                                node.error(err,msg);
                            }
                        });
