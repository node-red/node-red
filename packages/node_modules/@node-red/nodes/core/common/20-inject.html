...
<script type="text/html" data-template-name="inject">
    ...
    <div class="form-row cronti-form">
        ...
        <select id="inject-time-type-select">
            ...
            <option value="cronti-on-day" data-i18n="inject.cronti.on-day"></option>
    ...
    <!-- Cronti Begin -->
    <div class="form-row inject-time-row hidden cronti-form" id="inject-time-row-cronti-on-day">
        <span data-i18n="inject.cronti.date"></span>
        <input type="datetime-local" id="inject-time-cronti-on-day-date">
        <p class="clear"></p>
    </div>
    ...
    <div class="form-row inject-time-row hidden cronti-form" id="inject-time-row-cronti-on-week">
        ...
        <select id="inject-time-cronti-on-week-first-day-of-week">
            <option value=""></option>
            <option value="0" data-i18n="inject.cronti.monday"></option>
            <option value="1" data-i18n="inject.cronti.tuesday"></option>
            <option value="2" data-i18n="inject.cronti.wednesday"></option>
            <option value="3" data-i18n="inject.cronti.thursday"></option>
            <option value="4" data-i18n="inject.cronti.friday"></option>
            <option value="5" data-i18n="inject.cronti.saturday"></option>
            <option value="6" data-i18n="inject.cronti.sunday"></option>
        </select>
        <p class="clear"></p>
        <span data-i18n="inject.cronti.date"></span>
        ...
        oneditprepare: function() {
            ...
            if(crontiMethod && crontiArgs.length) {
                var crontiArgsInputId = {
                    "onDay": ["inject-time-cronti-on-day-date"],
                    ...
                    "onDate": ["inject-time-cronti-on-date"]
                };
                
                (crontiArgsInputId[crontiMethod] || []).forEach(function(input, i) {
                    $("#"+input).val(crontiArgs[i].replace(/(M)|(W)|(WD)|(FD)|(d)|(h)|(m)/g, ""));
                });
            }
            // Cronti End
        ...
        oneditsave: function() {
            ...
            if(isCronti) {
                ...
                switch (method) {
                    case "onDay":
                        args = [
                            $("#inject-time-cronti-on-day-date").val()
                        ];
                        break;
                    case "onWeek":
                        args = [
