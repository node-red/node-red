...
module.exports = function(RED) {
    ...
    function MQTTRequestNode(n) {
        ...
        function settleRequest(ctx, err) {
            ...
            if (err) {
                // route the clean, passed-through request msg (not the publish-mutated one) to any Catch node
        ...
        async function sendRequest(msg, send, done) {
            try {
                ...
                // 6. ensure we are subscribed to the response topic BEFORE publishing the request
                ...
                if (isStaticResponseTopic) {
                    ...
                    try {
                        await subscribeAsync(responseTopic, { qos: resolveQoS(msg) }, responseRouter, ctx.subRef)
                    } catch (subErr) {
                ...
                // 7. publish the request, then arm the timeout once it has been sent
                msg.topic = topic
                msg.responseTopic = responseTopic
                msg.correlationData = correlationData
                msg.qos = resolveQoS(msg)
                ...
                broker.publish(msg, function (err) {
                    ...
                    ctx.timeoutHandle = setTimeout(function () {
                        settleRequest(ctx, new Error(RED._("mqtt.errors.request-timed-out", { timeout: timeoutSec })))
                    }, timeoutSec * 1000)
                })
            } catch (err) {
