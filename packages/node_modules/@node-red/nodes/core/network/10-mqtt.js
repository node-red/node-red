...
module.exports = function(RED) {
     ...
     * *(Refactored from the "MQTT in" node for re-use by "MQTT in" and "MQTT control")*
     */
     ...
     * *(Refactored from the "MQTT out" node for re-use by "MQTT out" and "MQTT control")*
     * @param {MQTTControlNode|MQTTOutNode} node the owner node
    ...
    function MQTTBrokerNode(n) {
        ...
        node.setOptions = function(opts, init) {
            ...
            /** flag to indicate a new client ID is needed */
            let brokerChanged = init;
            ...
            if(originalBrokerURL !== node.brokerurl) {
                brokerChanged = true;
            }
            ...
            if(init || brokerChanged || !node.options.clientId) {
                node.options.clientId = node.clientid || 'nodered_' + (1+Math.random()*4294967295).toString(16);
            }
            ...
            if (node.compatmode == "true" || node.compatmode === true || node.protocolVersion == 3) {
                node.options.protocolId = 'MQIsdp';
    ...
    function MQTTInNode(n) {
        ...
        if (!isValidSubscriptionTopic(node.topic)) {
    ...
    //#region "MQTTControl node"
    function MQTTControlNode(n) {
        RED.nodes.createNode(this, n);

        /**@type {MQTTControlNode}*/const node = this;
        /**@type {string}*/node.broker = n.broker;
        /**@type {MQTTBrokerNode}*/node.brokerConn = RED.nodes.getNode(node.broker);
        /**@type {string}*/node.action = n.action;
        node.subs = {};
        ...
        if (node.brokerConn) {
            ...
            node.on('input', function (msg, send, done) {
                const v5 = node.brokerConn.options && node.brokerConn.options.protocolVersion == 5;
                const action = node.action === 'setby' ? msg.action : node.action;
                let actionData;
                ...
                if (action === Actions.CONNECT) {
                    actionData = typeof msg.options === 'object' ? msg.options : null;
                    if (node.brokerConn.canConnect()) {
                        if (actionData) {
                            node.brokerConn.setOptions(actionData);
                        }
                        node.brokerConn.connect();
                        send([null, msg]);
                        done();
                    } else {
                    ...
                    const subscriptions = [];
                    actionData = msg.subscription;
                    //coerce msg.subscription into an array of strings or objects (for later iteration)
                    if (Array.isArray(msg.subscription)) {
                        actionData = msg.subscription;
                    } else if (typeof msg.subscription == 'string' || typeof msg.subscription == 'object') {
                        actionData = [msg.subscription];
                    } else {
                        //node.error('invalid-action-badsubscription', msg);
                    ...
                    if (action === Actions.UNSUBSCRIBE) {
                        ...
                        subscriptions.forEach(function (sub) {
                            ...
                            node.subs[sub.topic] = sub; //save for later unsubscription & 'list' action
                        })
                        //send updated subs to 2nd output for user status
                        msg.payload = Object.values(node.subs);
                        node.send([null, msg]);
                        done();
                    }
        ...
        this.on('close', function (removed, done) {
            if (node.brokerConn) {
                Object.keys(node.subs).forEach(function (topic) {
    ...
    RED.nodes.registerType('mqtt control', MQTTControlNode);
    //#endregion "MQTTControl node"
