...
module.exports = function(RED) {
    ...
    function makeCorrelationData(value) {
    ...
    // Upper bound on the number of in-flight messages a node may buffer, from the `nodeMessageBufferMaxLength`
    // runtime setting (0 or unset = unlimited). Cached on first read, mirroring the split/join nodes.
    let _nodeMessageBufferMaxLength;
    function nodeMessageBufferMaxLength() {
        if (_nodeMessageBufferMaxLength === undefined) {
            const name = "nodeMessageBufferMaxLength";
            _nodeMessageBufferMaxLength = RED.settings.hasOwnProperty(name) ? RED.settings[name] : 0;
        }
        return _nodeMessageBufferMaxLength;
    }

    /**
     * Promisified wrapper around `RED.util.evaluateNodeProperty`.
     * @param {string} value The property value/expression to evaluate
     * @param {string} type The typedInput type (e.g. 'str', 'msg', 'num', 'jsonata')
     * @param {object} node The node evaluating the property
     * @param {object} msg The message being processed
     * @returns {Promise<*>} resolves with the evaluated value, rejects on error
     */
    function evaluateNodePropertyAsync(value, type, node, msg) {
     ...
     */
    function subscriptionHandler(node, datatype ,topic, payload, packet, options) {
        if (!packet) { packet = {}; }
        options = options || {};
        // When this is an MQTT Request message, the node passes its (cloned) request msg as options.requestMsg
        //  and the response is laid over it - preserving non-MQTT properties (req/res, _linkSource, _msgid, user props).
        //  First however, we need to clear any MQTT properties that may be present in the orignal request msg
        //  as they should represent the MQTT properties of the response (where present!).
        // Additionally, the original request topic is preserved as msg.requestTopic for convenience.
        const msg = options.requestMsg || {};
        if (options.requestMsg) {
            // clear the request's MQTT properties so they can't linger and masquerade as the response's
            // (requestTopic is NOT in this list - the caller sets it on the base to preserve it for reference)
            ['topic', 'payload', 'qos', 'retain', 'responseTopic', 'correlationData', 'contentType',
                'messageExpiryInterval', 'payloadFormatIndicator', 'reasonString', 'userProperties']
                .forEach((p) => { delete msg[p]; });
            msg.requestTopic = options.requestTopic;
        }
        msg.topic = topic;
        msg.qos = packet.qos;
    ...
    function MQTTBrokerNode(n) {
        ...
        node.queue = [];
        node.subscriptions = {};
        // In-flight MQTT v5 request/response correlations for `mqtt request` nodes that share this broker
        // (and therefore this client). Keyed by `responseTopic + '\0' + correlationData(base64)`; each entry
        // carries information about its owning node so any handler on the shared client routes the response to the right node.
        /** MQTT Request Node in-flight request map */
         ...
         * @param {*} callback
         * @param {*} ref
         * @param {function} [subscribeDone] Optional completion callback `(err, granted)` invoked once the
         *  broker confirms (SUBACK) or refuses the subscription. Called with an error if the client is not
         *  currently connected or the broker refuses the subscription (granted QoS 128). Existing callers that
         *  omit this argument keep the previous fire-and-forget behaviour.
         */
        node.subscribe = function (topic, options, callback, ref, subscribeDone) {
            /** @type {Subscription} */
            let subscription
            let doCompare = false
            ...
            if (node.connected) {
                ...
                if (typeof subscribeDone === "function") {
                    // caller wants confirmation - use the mqtt.js SUBACK callback
                    node.client.subscribe(topic, subscription.options, function (err, granted) {
                        if (err) { return subscribeDone(err) }
                        // granted is an array of { topic, qos }; qos 128 indicates the broker refused it
                        if (Array.isArray(granted)) {
                            const refused = granted.find(g => g && g.qos === 128)
                            if (refused) {
                                return subscribeDone(new Error(RED._("mqtt.errors.subscribe-failed", { topic: refused.topic || topic })))
                            }
                        }
                        subscribeDone(null, granted)
                    })
                } else {
                    node.client.subscribe(topic, subscription.options)
                }
            } else if (typeof subscribeDone === "function") {
                // not connected: the subscription is stored and will be (re)applied on connect,
                // but we cannot confirm it now, so report to callers that asked for confirmation
                subscribeDone(new Error(RED._("mqtt.errors.not-connected")))
            }
         ...
         * @param {Object} msg - The MQTT message to publish
         * @param {(err: Error | mqtt.ErrorWithReasonCode) => void} done - Optional callback to be called when the publish is complete or if there is an error
         */
    ...
    //#region "MQTTRequestNode node"
    function MQTTRequestNode(n) {
        RED.nodes.createNode(this,n)
        const node = this
        node.topic = n.topic
        ...
        node.retain = n.retain
        node.broker = n.broker
        node.datatype = n.datatype || 'auto-detect'
        node.responseTopicValue = n.responseTopic
        node.responseTopicType = n.responseTopicType || 'str'
        node.correlationDataValue = n.correlationData
        node.correlationDataType = n.correlationDataType || 'auto'
        node.timeout = (n.timeout !== undefined && n.timeout !== '') ? n.timeout : 5
        node.timeoutType = n.timeoutType || 'num'
        // number of this node's in-flight requests, bounded by nodeMessageBufferMaxLength (0 = unlimited)
        node.inflightCount = 0
        /** @type {MQTTBrokerNode} */
        ...
        // A static response topic is subscribed once at instantiation and kept for the node's lifetime
        // A dynamic (msg.) response topic is subscribed per-request and torn down after a matched response (or timeout)
        const isStaticResponseTopic = node.responseTopicType === 'str'

        // In-flight requests live on the shared broker connection (node.brokerConn.pendingRequests), not on
        // this node, so that request nodes sharing a broker cannot collide: two requests carrying the same
        // responseTopic + correlationData on the same client are indistinguishable on the wire, so the second
        // is rejected. Each entry carries its owning node, so whichever node's handler fires on the shared
        // client routes the response back to the correct node.
        function requestKey(responseTopic, correlationData) {
            return responseTopic + '::' + correlationData.toString('base64')
        }

        function cleanupSubscription(topic, ref) {
            if (node.brokerConn) {
        ...
        /**
         * Promisified node.brokerConn.subscribe that resolves only once the broker confirms the
         * subscription (SUBACK) and rejects if the broker is not connected or refuses the subscription.
         */
        function subscribeAsync(topic, options, handler, ref) {
            return new Promise((resolve, reject) => {
                if (!node.brokerConn) { return reject(new Error(RED._("mqtt.errors.missing-config"))) }
                if (!node.brokerConn.connected) { return reject(new Error(RED._("mqtt.errors.not-connected"))) }
                node.brokerConn.subscribe(topic, options, handler, ref, function (err) {
        ...
        /**
         * Resolve the QoS to use for the request publish and the response subscription.
         * Precedence: configured node.qos, then msg.qos, then a default. For the v5 request-response
         * pattern we default to QoS 1 (at-least-once) rather than 0: a QoS 0 response can be silently
         * dropped, which would surface only as a timeout. QoS 2 is intentionally NOT the default as it
         * is not universally supported (e.g. AWS IoT Core supports only QoS 0 and 1).
         * @param {object} [msg] the message being processed (omitted for the instantiation-time static sub)
         * @param {number} [defaultQos=1] the default QoS to use if neither node.qos nor msg.qos is set
         * @returns {number} the resolved QoS (0, 1, or 2)
         */
        function resolveQoS(msg, defaultQos = 1) {
            let q = (node.qos !== null && node.qos !== undefined && node.qos !== '') ? node.qos : (msg && msg.qos)
            q = parseInt(q, 10)
            if (isNaN(q) || q < 0 || q > 2) {
                q = defaultQos ?? 1
            }
        ...
        // build the timeout error - recording whether the request was confirmed delivered - and settle.
        // The `msg.mqtt` flag lets a downstream retry tell "delivered, no response" from "not confirmed sent".
        function onRequestTimeout(ctx, timeoutSec) {
            const delivered = ctx.published === true
            if (ctx.requestMsg) {
                ctx.requestMsg.mqtt = { timedOut: true, qos: ctx.qos, delivered }
            }
            const key = delivered ? "mqtt.errors.request-timed-out-delivered" : "mqtt.errors.request-timed-out-undelivered"
            settleRequest(ctx, new Error(RED._(key, { timeout: timeoutSec })))
        }

        // resolve/reject a pending request exactly once and tidy up its timer & (dynamic) subscription.
        // Everything is driven off ctx (which carries its owning node), because on a shared broker the
        // handler that fires may belong to a different node than the one that made the request.
        function settleRequest(ctx, err) {
            if (!ctx) { return }
            ...
            ctx.settled = true
            if (node.inflightCount > 0) { node.inflightCount-- }

            if (node.brokerConn?.pendingRequests) {
                node.brokerConn.pendingRequests.delete(ctx.key)
            }
            if (!ctx.isStatic && node.brokerConn) {
                // dynamic response topics are ALWAYS torn down after a matched response or timeout
                node.brokerConn.unsubscribe(ctx.topic, ctx.subRef, true)
            }
            if (err) {
                // Purge a QoS>0 request still stuck (unacknowledged) in the client's outgoing store so it isn't
                // retransmitted after a reconnect (messageExpiryInterval is the broker-side backstop). Only do
                // this while UNACKED (!ctx.published): once acked the packet is gone and its message id has been
                // freed - and could since have been reused by another publish, so removing it would be unsafe.
                if (!ctx.published && ctx.messageId != null && node.brokerConn && node.brokerConn.client && typeof node.brokerConn.client.removeOutgoingMessage === "function") {
                    node.brokerConn.client.removeOutgoingMessage(ctx.messageId)
                }
                // We use node.error instead of ctx.done() so that we can route the clean (cloned from input msg) message
                // to any Catch node (this msg gets the msg.mqtt outcome flag set in onRequestTimeout)
                node.error(err, ctx.requestMsg || {})
            } else {
                ctx.done()
            }
        }

        // emit the matched response on the owning node and settle the associated request. The response is laid
        // over the original (cloned) request msg: response payload -> msg.payload, response metadata onto the
        // owned MQTT fields, arrival topic -> msg.topic, and the original request topic kept as msg.requestTopic.
        function deliverResponse(ctx, topic, payload, packet) {
            if (ctx.settled) { return }
            try {
                subscriptionHandler(node, ctx.datatype, topic, payload, packet, {
                    requestMsg: ctx.requestMsg,
                    requestTopic: ctx.requestTopic
                })
                settleRequest(ctx)
            } catch (err) {
                settleRequest(ctx, err)
            }
        }

        // Shared response handler used for both static and dynamic response-topic subscriptions. It looks the
        // request up in the broker-wide map by (responseTopic + correlationData) and delivers it to the owner.
        function responseRouter(topic, payload, packet) {
            const props = (packet && packet.properties) || {}
            const correlationData = makeCorrelationData(props.correlationData)
            if (!correlationData) { return }
            if (!node.brokerConn || !node.brokerConn.pendingRequests) { return }
            const key = requestKey(topic, correlationData)
            const ctx = node.brokerConn.pendingRequests.get(key)
            if (!ctx) { return } // no in-flight request for this topic+correlation (or already settled)
            // Check that the handler firing is for the node that owns the request & deliver it (or not)
            if ((node.id + '-' + key) === ctx.subRef) {
                deliverResponse(ctx, topic, payload, packet)
            }
        }

        async function sendRequest(msg, send, done) {
            try {
                // 1. resolve topics
                const topic = await evaluateNodePropertyAsync(node.topic, node.topicType || 'msg', node, msg)
                if (typeof topic !== 'string' || topic.trim() === '' || !isValidPublishTopic(topic)) {
                    return done(new Error(RED._("mqtt.errors.invalid-topic")))
                }

                let responseTopic
                if (isStaticResponseTopic) {
                    responseTopic = node.responseTopicValue
                } else {
                    responseTopic = await evaluateNodePropertyAsync(node.responseTopicValue, node.responseTopicType, node, msg)
                }
                if (typeof responseTopic !== 'string' || responseTopic.trim() === '' || !isValidPublishTopic(responseTopic)) {
                    return done(new Error(RED._("mqtt.errors.invalid-responseTopic")))
                }

                // 2. resolve the timeout (in seconds)
                const timeoutSec = Number(await evaluateNodePropertyAsync(node.timeout, node.timeoutType || 'num', node, msg))
                if (isNaN(timeoutSec) || timeoutSec <= 0) {
                    return done(new Error(RED._("mqtt.errors.invalid-timeout")))
                }

                // 3. resolve the correlation data (auto-generated uuid, or a msg property)
                //    Additionally, check if there is already an in-flight request with this correlation (reject if so)
                let correlationId
                if (node.correlationDataType === 'msg') {
                    const provided = await evaluateNodePropertyAsync(node.correlationDataValue, 'msg', node, msg)
                    correlationId = (provided !== undefined && provided !== null && provided !== '') ? provided : uuid()
                } else {
                    correlationId = uuid()
                }
                const correlationData = makeCorrelationData(correlationId)
                if (!correlationData) {
                    return done(new Error(RED._("mqtt.errors.invalid-correlationData")))
                }
                // Key on responseTopic + correlationData, scoped to the shared broker connection. Two requests
                // that would put the same pair on the same client are indistinguishable on the wire, so reject
                // the second rather than let their responses cross.
                const key = requestKey(responseTopic, correlationData)
                const broker = node.brokerConn
                if (broker.pendingRequests.has(key)) {
                    return done(new Error(RED._("mqtt.errors.duplicate-correlationData")))
                }

                // 4. cap the number of concurrent in-flight requests this node will buffer
                const maxInflight = nodeMessageBufferMaxLength()
                if (maxInflight > 0 && node.inflightCount >= maxInflight) {
                    return done(new Error(RED._("mqtt.errors.too-many-requests", { count: maxInflight })))
                }

                // 5. build the request context and register it on the broker-wide map.
                //    Snapshot a clone of the incoming msg (before the publish-time mutations)
                //    - the response will be overlaid on it, and the msg routed to Catch on error.
                //    Cloning (rather than holding a reference) isolates it from in-flight/by-reference changes;
                //    cloneMessage keeps req/res/_linkSource so http-in/link-call style flows still work downstream.
                const qos = resolveQoS(msg)
                const ctx = {
                    key,
                    datatype: node.datatype,
                    isStatic: isStaticResponseTopic,
                    topic: responseTopic,
                    requestTopic: topic,
                    requestMsg: RED.util.cloneMessage(msg),
                    subRef: node.id + '-' + key,
                    correlationData,
                    qos,
                    published: false,   // set true once publishing is confirmed (QoS>0: broker PUBACK/PUBCOMP)
                    messageId: null,    // mqtt.js outgoing message id (QoS>0) - lets us purge on give-up
                    timeoutHandle: null,
                    settled: false,
                    done,
                }
                broker.pendingRequests.set(key, ctx)
                node.inflightCount++

                // 6. arm the timeout NOW (end-to-end). It covers subscribe + publish + awaiting the response, so
                //    the node produces exactly one outcome within `timeout` even if the connection is half-open
                //    and the subscribe/publish never completes.
                ctx.timeoutHandle = setTimeout(function () { onRequestTimeout(ctx, timeoutSec) }, timeoutSec * 1000)

                // 7. ensure we are subscribed to the response topic BEFORE publishing the request
                //    (settleRequest on any failure path so the map entry and in-flight count are released)
                if (isStaticResponseTopic) {
                    // static topic is already subscribed (at instantiation) - just verify we can publish
                    if (!broker.connected) {
                        settleRequest(ctx, new Error(RED._("mqtt.errors.not-connected")))
                        return
                    }
                } else {
                    try {
                        await subscribeAsync(responseTopic, { qos }, responseRouter, ctx.subRef)
                    } catch (subErr) {
                        settleRequest(ctx, subErr)
                        return
                    }
                    if (ctx.settled) { return } // timed out (or otherwise settled) while subscribing
                }

                // 8. publish the request. The timeout is already running; the publish callback only records the
                //    outcome so a subsequent timeout can report whether the request was confirmed delivered.
                msg.topic = topic
                msg.responseTopic = responseTopic
                msg.correlationData = correlationData
                msg.qos = qos
                msg.retain = false // opinionated - retain is not sensible for request-response
                msg.messageExpiryInterval = Math.ceil(timeoutSec) // tie the request's broker-side lifetime to our timeout
                broker.publish(msg, function (err) {
                    if (ctx.settled) { return } // already timed out / settled - ignore the late publish result
                    if (err) {
                        settleRequest(ctx, err)
                        return
                    }
                    ctx.published = true // QoS>0: broker acknowledged receipt; QoS0: written to the connection
                })
                // Capture the outgoing message id (QoS>0) now (synchronously), before any PUBACK - so a give-up
                // can still purge a packet that is stuck (unacked) in the client's outgoing store. Reading it
                // in the publish callback would be too late: by ACK time the packet is already out of the store.
                if (qos > 0 && node.brokerConn && node.brokerConn.client && typeof node.brokerConn.client.getLastMessageId === "function") {
                    ctx.messageId = node.brokerConn.client.getLastMessageId()
                }
            } catch (err) {
                done(err)
            }
        ...
        if (node.brokerConn) {
            // This node relies entirely on the MQTT v5 request/response pattern. If the configured broker
            // is not v5, don't register with it (so the broker's connection status can't mask this warning),
            // surface a clear status, and reject any request that arrives.
            if (!node.brokerConn.v5()) {
                node.status({ fill: "red", shape: "ring", text: "node-red:mqtt.label.requiresV5" })
                node.on("input", function (msg, send, done) {
                    done(new Error(RED._("mqtt.errors.requires-v5")))
                })
                node.on("close", function (removed, done) {
                    // never registered / never subscribed when the broker isn't v5, so nothing to tidy up
                    node.brokerConn = null
                    done()
                })
                return
            ...
            // static response topic: subscribe once at instantiation. The subscription is stored by the
            // broker and (re)applied automatically on (re)connect. Responses are routed by correlation data.
            if (isStaticResponseTopic && node.responseTopicValue && isValidPublishTopic(node.responseTopicValue)) {
                node.staticSubRef = node.id + '-static'
                // no msg available at instantiation - QoS derives from node.qos (default 1)
                node.brokerConn.subscribe(node.responseTopicValue, { qos: resolveQoS() }, responseRouter, node.staticSubRef)
            }
            ...
            node.on('close', function(removed, done) {
                // tidy up this node's own in-flight requests (timers, and dynamic subscriptions) from the
                // broker-wide map, leaving other nodes' requests intact. Ownership is encoded in ctx.subRef
                // (node.id + '-' + key), so only remove entries belonging to this node.
                if (node.brokerConn && node.brokerConn.pendingRequests) {
                    const ownerPrefix = node.id + '-'
                    node.brokerConn.pendingRequests.forEach(function (ctx, key) {
                        if (!ctx.subRef || ctx.subRef.indexOf(ownerPrefix) !== 0) { return }
                        if (ctx.timeoutHandle) { clearTimeout(ctx.timeoutHandle) }
                        if (!ctx.isStatic) { cleanupSubscription(ctx.topic, ctx.subRef) }
                        node.brokerConn.pendingRequests.delete(key)
                    })
                }
                node.inflightCount = 0
                if (node.staticSubRef) {
                    cleanupSubscription(node.responseTopicValue, node.staticSubRef)
                }
                if (node.brokerConn) {
                    node.brokerConn.deregister(node, done, removed)
                    node.brokerConn = null
    ...
    RED.nodes.registerType("mqtt request",MQTTRequestNode)
    //#endregion "MQTTRequestNode node"
}
