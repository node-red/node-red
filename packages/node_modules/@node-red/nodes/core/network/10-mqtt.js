...
module.exports = function(RED) {
    ...
    function subscriptionHandler(node, datatype ,topic, payload, packet) {
        if (!packet) { packet = {}; }
        // For MQTT Request-Response pattern, there are 2 topics we need to preserve (`topic` and `responseTopic`) in the message we send.
        // Below allows topic to be a string (backwards compatible) or an object (with both `topic` and `responseTopic`)
        let topics = { topic };
        if (typeof topic === "object" && topic.topic) {
            topics.responseTopic = topic.responseTopic;
            topics.topic = topic.topic;
        }
        const msg = {...topics, payload:null, qos:packet.qos, retain:packet.retain};
    ...
    function MQTTRequestNode(n) {
        ...
        // emit the matched response on the owning node and settle the associated request
        function deliverResponse(ctx, topic, payload, packet) {
            if (ctx.settled) { return }
            try {
                subscriptionHandler(node, ctx.datatype, { topic: ctx.requestTopic, responseTopic: ctx.topic }, payload, packet)
                settleRequest(ctx)
            } catch (err) {
