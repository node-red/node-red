...
<style>
    ...
    .form-row.form-row-tip > .form-tips {
        width: calc(70% - 18px);
        display: inline-block;
        margin-top: -8px;
    }
    .form-tips.form-tips-warning {
        border-color: var(--red-ui-form-input-border-error-color)
    }
    .form-row.label-length-120 > label {
        width: 120px;
    }
...
<script type="text/html" data-template-name="mqtt in">
    ...
    <div class="form-row form-row-tip form-row-mqtt-datatype-tip">
        <label> &nbsp; </label>
...
<script type="text/html" data-template-name="mqtt request">
    <div class="form-row label-length-120">
        <label for="node-input-broker"><i class="fa fa-globe"></i> <span data-i18n="mqtt.label.broker"></span></label>
        <input type="text" id="node-input-broker">
    </div>
    <div class="form-row label-length-120 form-row-mqtt-request-v5tip" style="display:none;">
        <div class="form-tips form-tips-warning"><i class="fa fa-warning"></i> <span data-i18n="mqtt.label.requiresV5Tip"></span></div>
    </div>
    <div class="form-row label-length-120">
        <label for="node-input-topic"><i class="fa fa-tasks"></i> <span data-i18n="common.label.topic"></span></label>
        <input type="text" id="node-input-topic" data-i18n="[placeholder]common.label.topic" style="width: 70%;">
        <input type="hidden" id="node-input-topicType">
    </div>

    <div class="form-row label-length-120">
        <label for="node-input-responseTopic"><i class="fa fa-tasks"></i> <span data-i18n="mqtt.label.responseTopic"></span></label>
        <input type="text" id="node-input-responseTopic" data-i18n="[placeholder]mqtt.label.responseTopic" style="width: 70%;">
        <input type="hidden" id="node-input-responseTopicType">
    </div>
    <div class="form-row label-length-120 form-row-tip form-row-mqtt-request-response-topic-tip">
        <label> &nbsp; </label>
        <div class="form-tips"><span>Using a static response topic is highly recommended for performance.</span></div>
    </div>

    <div class="form-row label-length-120">
        <label for="node-input-correlationData"><i class="fa fa-link"></i> <span data-i18n="mqtt.label.correlationData"></span></label>
        <input type="text" id="node-input-correlationData" data-i18n="[placeholder]mqtt.label.correlationData" style="width: 70%;">
        <input type="hidden" id="node-input-correlationDataType">
    </div>
    <div class="form-row label-length-120 form-row-tip form-row-mqtt-request-correlation-tip">
        <label> &nbsp; </label>
        <div class="form-tips"><span>To avoid collisions, ensure each request has a unique correlation string.</span></div>
    </div>
    <div class="form-row label-length-120 mqtt-form-row-cols2">
        <label for="node-input-qos" class="mqtt-form-row-col1"><i class="fa fa-empire"></i> <span data-i18n="mqtt.label.qos"></span></label>
        <select id="node-input-qos" style="width:125px !important" class="mqtt-form-row-col1">
            <option value=""></option>
            <option value="0">0</option>
            <option value="1">1</option>
            <option value="2">2</option>
    ...
    <div class="form-row label-length-120">
        <label for="node-input-timeout"><i class="fa fa-clock-o"></i> <span data-i18n="mqtt.label.expiry"></span></label>
        <input type="text" id="node-input-timeout" style="width: 70%;">
    ...
    <div class="form-row label-length-120 form-row-tip form-row-mqtt-datatype-tip">
        <label> &nbsp; </label>
        <div class="form-tips"><span data-i18n="mqtt.label.auto-mode-depreciated"></span></div>
    </div>
...
(function() {
    ...
    function validateMQTTPublishTopicTyped (type, value, opt) {
        if (!value.trim()) { return RED._('mqtt.errors.invalid-topic'); }
        let result = true
        if (type === 'msg' || type === 'env') {
            result = RED.utils.validatePropertyExpression(value, { label: RED._('mqtt.errors.invalid-msg-property') })
        } else {
            result = validateMQTTPublishTopic(value)
        }
        return result
    }
    ...
    RED.nodes.registerType('mqtt request',{
        category: 'network',
        defaults: {
            name: { value: "" },
            topic: { value: "", required: true, validate: function (v, opt) {
                    console.log("[mqtt request] validate topic", v);
                    const optionType = $("#node-input-topicType").val()
                    return validateMQTTPublishTopicTyped(optionType, v, opt)
                }
            },
            topicType: { value: "str" },
            qos: { value: "" },
            datatype: { value: "auto-detect", required: true },
            responseTopic: { value: "", required: true, validate: function (v, opt) {
                    console.log("[mqtt request] validate response topic", v);
                    const optionType = $("#node-input-responseTopicType").val()
                    return validateMQTTPublishTopicTyped(optionType, v, opt)
                }
            },
            responseTopicType: { value: 'str' },
            correlationData: { value: "correlationData" },
            correlationDataType: { value: 'auto' },
            timeout: { value: 5, required: true },
            timeoutType: { value: 'num' },
            broker: {
                type: "mqtt-broker", required: true,
                label: RED._("node-red:mqtt.label.broker")
            }
        },
        color: "#d8bfd8",
        ...
        align: "right",
        label: function() {
            const altName = this.topicType === "str" || $("#node-input-topicType").val() === "str" ? this.topic : "mqtt request"
            return this.name || altName
        },
        oneditprepare: function() {
            let isOpen = false;
            console.log("[mqtt request] on edit prepare");
            setupTopicField(this, $("#node-input-topic"));
            // ### v5 warning ###
            $("#node-input-broker").on("change",function(d){
                const confNode = RED.nodes.node($("#node-input-broker").val());
                const v5 = confNode && confNode.protocolVersion == "5";
                // This node relies entirely on the MQTT v5 request/response pattern, so warn when the
                // selected broker is not v5 (only once a broker has actually been chosen).
                $("div.form-row-mqtt-request-v5tip").toggle(!!confNode && !v5);
            });
            $("#node-input-broker").trigger("change");

            // ### topic typed input ###
            const topicTypedInput = $("#node-input-topic").typedInput({
                default: 'str',
                types: ['str', 'msg', 'env'],
                typeField: $("#node-input-topicType")
            });
            // help user by populating default topic value when switching to 'msg' type
            $('#node-input-topic').on("change", function() {
                const ti = $(this);
                if (isOpen && ti.typedInput('type') === "msg" && !ti.typedInput('value')) {
                    ti.typedInput('value', 'topic');
                }
            });
            $('#node-input-topic').trigger("change");

            // ### response topic typed input ###
            const responseTopicTypedInput = $("#node-input-responseTopic").typedInput({
                default: 'str',
                types: ['str', 'msg', 'env'],
                typeField: $("#node-input-responseTopicType")
            });
            // when user changes response topic type to 'msg', ensure the value is populated with the default prop name & show the tip
            $('#node-input-responseTopic').on("change", function() {
                const ti = $(this);
                if (ti.typedInput('type') === "msg") {
                    if (isOpen && !ti.typedInput('value')) {
                        ti.typedInput('value', 'responseTopic');
                    }
                    $(".form-row-mqtt-request-response-topic-tip").show();
                } else {
                    $(".form-row-mqtt-request-response-topic-tip").hide();
                }
            })
            $('#node-input-responseTopic').trigger("change")

            // ### timeout typed input ###
            const timeoutTypedInput = $("#node-input-timeout").typedInput({
                default: 'num',
                types: ['num', 'msg', 'env'],
                typeField: $("#node-input-timeoutType")
            });
            // when user changes timeout type to 'msg', ensure the value is populated with the default prop name
            $('#node-input-timeout').on("change", function() {
                const ti = $(this);
                if (isOpen && ti.typedInput('type') === "msg" && !ti.typedInput('value')) {
                    ti.typedInput('value', 'messageExpiryInterval')
                }
            })

            // ### correlation data typed input ###
            // can be either auto-generated (a uuid (default)) or taken from a msg property.
            const correlationDataAutoOpt = { value: "auto", label: RED._("node-red:mqtt.label.correlationDataAuto"), hasValue: false };
            const correlationDataTypedInput = $("#node-input-correlationData").typedInput({
                default: 'auto',
                types: [correlationDataAutoOpt, 'msg'],
                typeField: $("#node-input-correlationDataType")
            });
            // when user changes correlation data type to 'msg' populate the value with the default prop name & show the tip
            $('#node-input-correlationData').on("change", function() {
                const ti = $(this);
                if (ti.typedInput('type') === "msg") {
                    if (isOpen && !ti.typedInput('value')) {
                        ti.typedInput('value', 'correlationData');
                    }
                    $(".form-row-mqtt-request-correlation-tip").show();
                } else {
                    $(".form-row-mqtt-request-correlation-tip").hide();
                }
            })
            $('#node-input-correlationData').trigger("change");

            // ### datatype ###
            $("#node-input-datatype").on("change", function() {
                const ti = $(this);
                if(ti.val() === "auto") {
                    $(".form-row-mqtt-datatype-tip").show();
                } else {
                    $(".form-row-mqtt-datatype-tip").hide();
                }
            })
            $("#node-input-datatype").trigger("change");
        ...
        oneditsave: function() {
            // No special conversion needed; typedInput type/value are stored automatically.
        },
        labelStyle: function() {
            return this.name?"node_label_italic":"";
        }
