...
<script type="text/html" data-template-name="mqtt control">
    <div class="form-row">
        <label for="node-input-broker"><i class="fa fa-globe"></i> <span data-i18n="mqtt.label.broker"></span></label>
        <input type="text" style="width: 70%;" id="node-input-broker">
    </div>
    <div class="form-row">
        <label for="node-input-action"><i class="fa fa-tasks"></i> <span data-i18n="mqtt.label.action"></span></label>
        <select type="text" id="node-input-action" style="width: 70%;">
            ...
            <option value="setby" data-i18n="mqtt.action-setby"></option>
        </select>
    </div>
    <div class="form-row">
        <label for="node-mqtt-control-action-tip">&nbsp;</label>
        <div id="node-mqtt-control-action-tip" class="form-tips" style="width: calc(70% - 12px); vertical-align: middle; max-width: unset; border-radius: 4px;  padding: 6px; display: inline-block"> </div>
    </div>
    <div class="form-row">
        <label for="node-input-name"><i class="fa fa-tag"></i> <span data-i18n="common.label.name"></span></label>
        <input type="text" style="width: 70%;" id="node-input-name" data-i18n="[placeholder]common.label.name">
    </div>
</script>
...
(function() {
    ...
    RED.nodes.registerType('mqtt control',{
        category: 'network',
        defaults: {
            name: {value:''},
            broker: {type:'mqtt-broker', required:true},
        ...
        icon: 'bridge.svg',
        label: function() {
            return this.name||'mqtt control';
        },
        labelStyle: function() {
            return this.name?'node_label_italic':'';
        },
        outputLabels: function(index) {
            return ["subscription messages", "control messages"][index];
        },
        oneditprepare() {
            console.log("MQTT Control - oneditprepare")
            var node = this;
            $('#node-input-action').on('change', function(el) {
                var text = node._('mqtt.action-tip.' + this.value);
                $('#node-mqtt-control-action-tip').text(text);
            });
        }
    });
