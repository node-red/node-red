...
<script type="text/html" data-help-name="mqtt control">
   ...
   "will": {
      ...
      <dd>
         msg.subscription should contain the topic, an array of topics, an object conatining topic and properties or an array of objects conatining topic and properties
         <h5>example 1...</h5>
   ...
   <h3>Output 2</h3>
   <dl class="message-properties">
      <h4>messages in response to an action</h4>
      ...
      <hr>
      <h5>"connect" and "disconnect" action messages will return the original msg</h5>
   </dl>   
   <h3>Details</h3>
   <p>A subscription topic can include MQTT wildcards, + for one level, # for multiple levels.</p>
   <p>This node requires a connection to a MQTT broker to be configured. This is configured by clicking
   the pencil icon.</p>
   <p>To make a dynamic connection, you can turn off the broker option "auto connect" then run the "connect" acction with a new broker connection url.</p>
   <p>Several MQTT nodes (in or out) can share the same broker connection if required.</p>
</script>
